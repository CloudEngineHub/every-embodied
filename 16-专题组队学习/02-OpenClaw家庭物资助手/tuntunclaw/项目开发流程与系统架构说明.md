...
- `manipulator_grasp/env/ur5_grasp_env.py`（运行环境中的 MuJoCo 封装；当前公开子集未包含该文件）
  MuJoCo 环境封装。负责：
  - 加载 XML 场景。
  - 初始化机械臂与夹爪。
  - 管理离屏渲染和被动 viewer。
...
- `manipulator_grasp/env/ur5_grasp_env.py` 中的 `render`
...
为解决这个问题，环境封装 `manipulator_grasp/env/ur5_grasp_env.py` 采用以下机制：
...
- [grasp_process.py](./grasp_process.py)
- [vlm_process.py](./vlm_process.py)
- `manipulator_grasp/env/ur5_grasp_env.py`（运行环境文件）
- [workflow_hooks.py](./workflow_hooks.py)
- [inventory.py](./inventory.py)
- [integrations.py](./integrations.py)
- [frontend/index.html](./frontend/index.html)
