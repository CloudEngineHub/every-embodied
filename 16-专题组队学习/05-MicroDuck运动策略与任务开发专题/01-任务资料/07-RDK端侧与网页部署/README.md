...
![Microduck 官方真机走路关键帧](assets/official_videos/microduck_real_walking_keyframes.jpg)

**视频 1 Microduck 官方真机走路片段。** 这段素材来自 Microduck RL 官方项目页的真机演示。仓库保留关键帧，不提交完整 MP4；这展示的是官方策略效果，不是本教程在本地重新训练收敛的结果。

![Microduck 4096 环境训练走路动态预览](assets/local_videos/microduck_4096env_6000iter_walk.gif)

**视频 2 本教程实际完成的 4096 环境强化学习结果。** 我们从 `microduck_rl` 的 `29e887e` 版本出发，在 `Mjlab-Velocity-Flat-MicroDuck` 上训练 6000 次 PPO 迭代，再用最终的 `model_5999.pt` 以固定 `0.4 m/s` 前进命令回放。录像关闭了测试阶段的外力推搡事件，但没有对 checkpoint 做后处理或剪接成功片段。页面展示压缩 GIF，完整视频不随主仓库提交。
...
![Microduck 命令编舞动态预览](assets/local_videos/microduck_command_dance.gif)

**视频 3 本教程用同一 walking policy 完成的命令编舞。** 这不是单独训练的 dance checkpoint，而是按时间改变 `vx / vy / yaw rate` 和四维头部姿态命令，让已经训练好的 61 维策略实时执行点头、侧移、左右转向、前后步和弧线动作。整段 12 秒视频来自连续闭环 rollout，没有逐帧修改机器人姿态。页面展示压缩 GIF，完整视频不随主仓库提交。

![RDK X5 作为策略大脑控制单只 Microduck](assets/local_videos/microduck_rdk_brain_single_preview.png)

**视频 4 RDK X5 策略大脑控制单只 Microduck。** Ubuntu 电脑只负责 MuJoCo 物理仿真与画面渲染；每个 50 Hz 控制周期生成的 61 维观测都通过局域网发给 RDK，RDK 运行 `microduck_policy_5999.onnx` 后返回 14 维动作。视频为连续 12 秒、600 个控制周期、360 帧的完整闭环，不是预先算好动作后再播放。仓库保留封面，完整视频不随主仓库提交。

![RDK X5 作为策略大脑控制 3x3 Microduck 方阵](assets/local_videos/microduck_rdk_brain_3x3_preview.png)

**视频 5 RDK X5 策略大脑控制 3×3 Microduck 方阵。** 九只鸭子位于同一个 MuJoCo world，每只都有独立的自由基座、关节状态、IMU 观测、历史动作和 BAM M6 XL330 执行器状态。Ubuntu 电脑把九组观测打包为一次网络请求，RDK 逐组执行 ONNX 并返回九组动作，再分别写回对应机器人。它验证的是单策略的批量边缘推理与多实例可视化，不代表九只机器人学会了通信或多智能体协同。仓库保留封面，完整视频不随主仓库提交。

![Microduck 三视角追逐同一颗球并连续踢三次](assets/local_videos/microduck_visual_soccer_continuous_chase_preview.png)

**视频 6A 三视角追逐同一颗球并连续踢三次。** 浏览器只在开局把球放置一次，之后不再改写球的位置、姿态或速度。鸭子完成第一脚后等待球在 MuJoCo 中自然滚动，再重新检测、追赶、对准并踢出第二脚和第三脚；因此这不是把球反复摆回脚边的三段动作拼接。视频开头短暂展示头部传感器第一视角和近距离跟随第二视角，随后切到默认第三视角，并在追逐、脚位对齐、BallKick 策略切换、触球和稳定收尾阶段持续把鸭子保持在画面中部。本轮三次均由状态机选择右脚，单脚触球窗口内足球平面位移约为 `0.112 m`、`0.288 m` 和 `0.376 m`；每次触球后控制权都返回 walking policy，机体全程保持直立。仓库保留封面，完整视频不随主仓库提交。

![Microduck 物理冲量侧倒与自动起身](assets/local_videos/microduck_physical_impulse_recovery_preview.png)
...
![Open Duck Mini v2 官方仿真走路关键帧](assets/official_videos/open_duck_mini_v2_sim_walking_keyframes.jpg)
...
在线演示中的球门不是一张背景图。物理层用两根圆柱门柱、一根横梁、两片侧网和一片后网组成静态 MuJoCo 碰撞体；显示层用同尺寸的 Three.js 管件与网格线建模。两层共享 `lineX`、`halfWidth`、`height`、`postRadius` 和目标点坐标，避免“画面已经进门，但物理判定使用另一条线”的错误。

视觉模块同时维护足球目标和球门目标。设球心为 `b`，选定的球门内目标为 `g`，则期望射门方向与球后准备点为：
...
控制器不会从机器人当前位置直接冲向球，而是先判断机器人是否位于 `-d` 一侧。若机器人在球前或球侧，则选择球后左/右绕行点；到达准备区后，再对齐 `theta_shot`，最后才进入选脚与近场接触控制。由于该 walking policy 没有可靠横移能力，绕行采用“原地转向 + 前进”的离散导航；距离球小于 `0.27 m` 时先退出到 `0.38 m`，用滞回清除贴球转圈的极限环。

进球判定也不是“球心碰到门线”就加分。只有下面三个条件同时满足，HUD 才显示 `SCORE 1 / VERIFIED OVER LINE`：
...
第一式要求**整颗球**越过门线，后两式要求球完整位于门柱和横梁内。球进入后由有碰撞的后网停止，AUTO 随即归零，便于检查最终状态。页面上的 `GOAL TARGET` 准星是该三维目标点投影到当前相机后的结果；教程版使用 MuJoCo 真值构造与检测器同形状的接口，真机可替换为球门关键点、语义分割或 AprilTag 定位。
...
| `circling-ball` | 机器人不在“球—球门”连线后方 | 选择一侧绕行点，避免从球前直接撞球 |
| `clearing-ball` | 重规划时距离球小于 `0.27 m` | 沿径向退出，超过 `0.38 m` 后重新规划 |
| `staging-shot` | 已在球后，但尚未到准备点 | 移动到距球约 `0.27 m` 的射门准备位 |
| `aligning-shot` | 准备位满足，但朝向偏离球门 | 对齐球心到目标点的射门方向 |
...
| `goal` | 整颗球越过门线且位于门框内 | 锁存得分并停止输出，保留球和机器人最终物理状态 |
