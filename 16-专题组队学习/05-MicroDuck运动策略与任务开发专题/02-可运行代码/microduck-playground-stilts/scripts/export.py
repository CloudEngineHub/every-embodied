...
def run_export(task_id: str, cfg: ExportConfig):
    ...
    if cfg.curriculum_step is not None:
        if cfg.curriculum_step < 0:
            raise ValueError("curriculum_step must be non-negative")
        # The runner checkpoint stores the curriculum counter for training,
        # but play/export intentionally creates a fresh environment.  Allow a
        # replay to start at a declared ladder stage instead of silently
        # falling back to stage A.
        env.reset()
        env.common_step_counter = int(cfg.curriculum_step)
