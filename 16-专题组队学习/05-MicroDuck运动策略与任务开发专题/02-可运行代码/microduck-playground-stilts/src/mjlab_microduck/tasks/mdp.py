...
from mjlab.envs.mdp.actions import (  # noqa: E402
    JointPositionAction as _JointAction,
    JointPositionActionCfg as _JointActionCfg,
)


@_dataclass(kw_only=True)
class LadderBootstrapJointPositionActionCfg(_JointActionCfg):
    """Position action that restores a captured bootstrap command after reset."""

    def build(self, env: ManagerBasedRlEnv):
        return LadderBootstrapJointPositionAction(self, env)


class LadderBootstrapJointPositionAction(_JointAction):
    """Keep the raw action consistent with a loaded bootstrap qpos.

    ManagerBasedRlEnv applies reset events before ActionManager.reset().  A
    plain event can therefore load qpos/qvel but its action-history write is
    cleared immediately afterward.  The term restores the pending normalized
    position action from the event during its own reset hook.
    """

    def _restore_bootstrap_action(self, env_ids: torch.Tensor | slice | None) -> None:
        pending = getattr(self._env, "_ladder_bootstrap_action", None)
        if pending is None:
            return
        pending = torch.as_tensor(pending, device=self.device, dtype=self._raw_actions.dtype)
        if pending.ndim == 1:
            pending = pending.unsqueeze(0).expand(self.num_envs, -1)
        if pending.shape != self._raw_actions.shape:
            raise ValueError(
                "bootstrap action shape does not match the ladder action term: "
                f"{tuple(pending.shape)} != {tuple(self._raw_actions.shape)}"
            )
        if env_ids is None:
            env_ids = slice(None)
        self._raw_actions[env_ids] = pending[env_ids]
        scale = torch.as_tensor(
            self._scale, device=self.device, dtype=self._raw_actions.dtype
        )
        offset = torch.as_tensor(
            self._offset, device=self.device, dtype=self._raw_actions.dtype
        )
        self._processed_actions[env_ids] = (
            self._raw_actions[env_ids] * scale + offset
        )
        manager_action = getattr(self._env.action_manager, "_action", None)
        if manager_action is not None and manager_action.shape == pending.shape:
            manager_action[env_ids] = pending[env_ids]

    def reset(self, env_ids: torch.Tensor | slice | None = None) -> None:
        super().reset(env_ids=env_ids)
        self._restore_bootstrap_action(env_ids)


@_dataclass(kw_only=True)
class LadderTeacherJointPositionActionCfg(_JointActionCfg):
    """Position action with an optional phase-conditioned footstep teacher.

    The teacher is only enabled when the ladder task explicitly supplies a
    solved support-pose file.  Normal walking and strict evaluation therefore
    keep the original 14-D policy action contract.
    """

    teacher_state_path: str
    teacher_target_rung: int = 2
    teacher_blend: float = 0.85
    teacher_duration: float = 1.20
    teacher_all_joints: bool = False

    def build(self, env: ManagerBasedRlEnv):
        return LadderTeacherJointPositionAction(self, env)


class LadderTeacherJointPositionAction(LadderBootstrapJointPositionAction):
    """Blend PPO residuals into a smooth, physically checked footstep target."""

    cfg: LadderTeacherJointPositionActionCfg

    def __init__(self, cfg: LadderTeacherJointPositionActionCfg, env: ManagerBasedRlEnv):
        super().__init__(cfg=cfg, env=env)
        def _load_joint_pos(path: str) -> torch.Tensor:
            payload = torch.load(path, map_location="cpu", weights_only=True)
            joint_pos = payload.get("joint_pos")
            if joint_pos is None:
                qpos = torch.as_tensor(payload["qpos"], dtype=torch.float32)
                joint_pos = qpos[7:]
            return torch.as_tensor(joint_pos, dtype=torch.float32)

        teacher_joint_pos = _load_joint_pos(cfg.teacher_state_path)
        if teacher_joint_pos.numel() != self.action_dim:
            raise ValueError(
                "ladder teacher joint_pos dimension does not match the action "
                f"dimension ({teacher_joint_pos.numel()} != {self.action_dim})"
            )
        self._teacher_joint_pos = teacher_joint_pos.to(self.device).reshape(1, -1)
        apex_path = os.environ.get("MICRODUCK_LADDER_TEACHER_APEX_STATE")
        self._teacher_apex_joint_pos = None
        if apex_path:
            apex_joint_pos = _load_joint_pos(apex_path)
            if apex_joint_pos.numel() != self.action_dim:
                raise ValueError(
                    "ladder apex teacher joint_pos dimension does not match the "
                    f"action dimension ({apex_joint_pos.numel()} != {self.action_dim})"
                )
            self._teacher_apex_joint_pos = apex_joint_pos.to(self.device).reshape(1, -1)
        self._teacher_start = torch.zeros(
            (self.num_envs, self.action_dim), device=self.device
        )
        self._teacher_last_target = torch.full(
            (self.num_envs,), -1, dtype=torch.long, device=self.device
        )
        action_joint_names = [
            self._entity.joint_names[int(index)]
            for index in self._target_ids.detach().cpu().tolist()
        ]
        left_leg_names = {
            "left_hip_yaw",
            "left_hip_roll",
        ...
        self._left_leg_action_mask = torch.tensor(
            [name in left_leg_names for name in action_joint_names],
            device=self.device,
            dtype=torch.float32,
        )
        self._right_leg_action_mask = torch.tensor(
            [name in right_leg_names for name in action_joint_names],
            device=self.device,
            dtype=torch.float32,
        )
        self._teacher_debug_steps = 0
        if os.environ.get("MICRODUCK_LADDER_TEACHER_DEBUG") == "1":
            print(
                "[ladder-teacher] enabled",
                "target_rung=",
                cfg.teacher_target_rung,
                "blend=",
                cfg.teacher_blend,
                "duration=",
                cfg.teacher_duration,
                "all_joints=",
                cfg.teacher_all_joints,
                "apex=",
                self._teacher_apex_joint_pos is not None,
            )

    def reset(self, env_ids: torch.Tensor | slice | None = None) -> None:
        super().reset(env_ids=env_ids)
        if env_ids is None:
            self._teacher_last_target[:] = -1
        else:
            self._teacher_last_target[env_ids] = -1

    def process_actions(self, actions: torch.Tensor) -> None:
        super().process_actions(actions)

        target = getattr(self._env, "_ladder_target_rung", None)
        if target is None:
            return
        target = target.to(device=self.device, dtype=torch.long)
        phase_time = getattr(self._env, "_ladder_phase_time", None)
        if phase_time is None:
            phase_time = torch.zeros(self.num_envs, device=self.device)

        current = self._entity.data.joint_pos[:, self._target_ids]
        new_target = target != self._teacher_last_target
        if new_target.any():
            self._teacher_start[new_target] = current[new_target]
        self._teacher_last_target[:] = target

        alpha = torch.clamp(
            phase_time / max(float(self.cfg.teacher_duration), 1e-4), 0.0, 1.0
        )
        smooth = alpha * alpha * (3.0 - 2.0 * alpha)
        if self._teacher_apex_joint_pos is None:
            desired = self._teacher_start + smooth[:, None] * (
                self._teacher_joint_pos - self._teacher_start
            )
        else:
            # The first segment lifts the swing foot over the intermediate
            # rung; only the second segment descends onto the target rung.
            apex_fraction = 0.55
            to_apex = torch.clamp(smooth / apex_fraction, 0.0, 1.0)
            from_apex = torch.clamp(
                (smooth - apex_fraction) / (1.0 - apex_fraction), 0.0, 1.0
            )
            apex_pose = self._teacher_start + to_apex[:, None] * (
                self._teacher_apex_joint_pos - self._teacher_start
            )
            final_pose = self._teacher_apex_joint_pos + from_apex[:, None] * (
                self._teacher_joint_pos - self._teacher_apex_joint_pos
            )
            desired = torch.where(
                (smooth < apex_fraction)[:, None], apex_pose, final_pose
            )

        active = target == int(self.cfg.teacher_target_rung)
        blend = (
            torch.as_tensor(
                float(self.cfg.teacher_blend), device=self.device, dtype=torch.float32
            )
            .clamp(0.0, 1.0)
            * active.float()
        )[:, None]
        # ``desired`` is an absolute joint-position target, while the policy
        # emits normalized actions consumed by ``processed = raw * scale +
        # offset``.  Convert back to the raw action space before blending;
        # treating the absolute pose as a raw action moves the legs to the
        # wrong side of the bridge pose whenever the configured scale is not 1.
        scale = torch.as_tensor(
            self._scale, device=desired.device, dtype=desired.dtype
        ).clamp_min(1e-6)
        offset = torch.as_tensor(
            self._offset, device=desired.device, dtype=desired.dtype
        )
        teacher_raw = (desired - offset) / scale
        # The default teacher only moves the planned swing leg so it cannot
        # unload a validated support foot.  Dynamic transfer experiments may
        # opt into a full-body joint reference: this is useful when the root
        # must lean over the support foot before the new rung can be loaded.
        if self.cfg.teacher_all_joints:
            swing_mask = torch.ones_like(actions)
        else:
            swing_id = (target + _LADDER_FIRST_SWING_FOOT) % 2
            swing_mask = torch.where(
                swing_id[:, None] == 0,
                self._left_leg_action_mask[None, :],
                self._right_leg_action_mask[None, :],
            )
        mixed = actions + blend * swing_mask * (teacher_raw - actions)
        self._raw_actions[:] = mixed
        self._processed_actions = mixed * self._scale + self._offset
        ...
        if (
            os.environ.get("MICRODUCK_LADDER_TEACHER_DEBUG") == "1"
            and self._teacher_debug_steps < 8
        ):
            print(
                "[ladder-teacher-step]",
                self._teacher_debug_steps,
                "target=",
                target[:1].detach().cpu().tolist(),
                "phase=",
                phase_time[:1].detach().cpu().tolist(),
                "active=",
                active[:1].detach().cpu().tolist(),
                "current=",
                current[:1].detach().cpu().tolist(),
                "desired=",
                desired[:1].detach().cpu().tolist(),
                "mixed=",
                mixed[:1].detach().cpu().tolist(),
            )
            self._teacher_debug_steps += 1
...
def _ladder_progress_potential(
    ...
    """Monotonic route potential used to shape the ladder climb."""
    asset: Entity = env.scene[asset_cfg.name]
    ...
    x_span = max(finish_x - start_x, 1e-6)
    x_progress = torch.clamp((x - start_x) / x_span, 0.0, 1.0)
...
def ladder_path_progress(
    ...
    """Potential difference for forward-and-up progress toward the platform."""
    potential = _ladder_progress_potential(
        env, start_x=start_x, finish_x=finish_x, platform_z=platform_z, asset_cfg=asset_cfg
    )
...
def ladder_height_progress(
    ...
    """Potential difference for gaining height, capped at platform height."""
    asset: Entity = env.scene[asset_cfg.name]
    z = torch.nan_to_num(
        asset.data.root_link_pos_w[:, 2] - env.scene.terrain.env_origins[:, 2], nan=0.0
    )
...
def ladder_foot_alignment(
    ...
    finish_x: float = 0.82,
    finish_z: float = 0.55,
    rail_half_width: float = 0.19,
    rung_count: int = 12,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Dense shaping for placing feet on the ladder's discrete rungs.

    Route and height potentials alone are too sparse for the first transition
    from flat-ground walking to a steep ladder.  This term shapes a scalar
    reward from the foot sites and fixed task geometry; it does not expose
    object IDs or contact labels to the actor.  The MuJoCo contact solver still
    decides whether a foot truly supports the body.
    """
    asset: Entity = env.scene[asset_cfg.name]
    origin = env.scene.terrain.env_origins
    ...
    rung_slots = max(rung_count - 1, 1)
    nearest = torch.round(t * rung_slots) / rung_slots
    rung_along = nearest * length
    rung_z = start_z + nearest * (finish_z - start_z)

    # A foot should be between the rails, near the ladder plane, and near a
    # discrete rung.  These scales are forgiving during bootstrap; collision
    # resolution still determines whether the foot truly supports the body.
    ...
    root_x = torch.nan_to_num(
        asset.data.root_link_pos_w[:, 0] - origin[:, 0], nan=0.0
    )
...
_LADDER_FEET_CFG = SceneEntityCfg(
    "robot", site_names=("left_foot", "right_foot")
)
# The fixed reset pose naturally discovers the right-foot first step.  Keep
# this explicit, then alternate feet for every subsequent planned rung.
_LADDER_FIRST_SWING_FOOT = 1
# A physically plausible biped ladder sequence loads both feet on the current
# rung before advancing to the next rung.  The plan index therefore counts
# foothold placements, while the scene still contains only ``rung_count``
# discrete rungs.  Keep an opt-out for reproducing the old one-foot-per-rung
# baseline, but use the double-support plan for V3 by default.
_LADDER_DOUBLE_SUPPORT = os.environ.get(
    "MICRODUCK_LADDER_DOUBLE_SUPPORT", "1"
) != "0"
# The lower timber in the reference scene is a real, raised entry foothold.
# Count its two alternating foot placements explicitly before the first rung;
# otherwise the planner asks the duck to jump from the floor directly onto a
# rung that overlaps the timber in x and creates a bad collision-local optimum.
LADDER_ENTRY_FOOTSTEPS = 2
_LADDER_ENTRY_FOOT_SITE_X_OFFSET = 0.02
# ``start_z`` is the first rung centre (0.035 m); the lower timber top is
# 0.025 m after the geometry correction, so its target site is just above that
# surface rather than at the rung height.
_LADDER_ENTRY_FOOT_SITE_Z_OFFSET = -0.010
# The planned point is the foot site, while MuJoCo contact is generated by the
# sole collision box below and slightly behind that site.  These offsets are
# calibrated against the reachable first rung in the video-aligned geometry.
_LADDER_FOOT_SITE_X_OFFSET = -0.032
_LADDER_FOOT_SITE_Z_OFFSET = 0.012


def _ladder_plan_count(rung_count: int) -> int:
    """Return the number of planned foot placements for this ladder."""
    return LADDER_ENTRY_FOOTSTEPS + (
        rung_count * 2 if _LADDER_DOUBLE_SUPPORT else rung_count
    )


def _ladder_entry_target_mask(target: torch.Tensor) -> torch.Tensor:
    """Return which planned targets are on the physical lower timber."""
    return target < LADDER_ENTRY_FOOTSTEPS


def _ladder_target_rung_index(
    target: torch.Tensor, rung_count: int
) -> torch.Tensor:
    """Map a planned foothold index to its physical rung index."""
    plan_count = _ladder_plan_count(rung_count)
    target = target.clamp(min=0, max=max(plan_count - 1, 0)).long()
    # Entry targets are represented by rung 0 for tensor indexing.  Callers
    # that need the actual contact type branch on _ladder_entry_target_mask.
    target = (target - LADDER_ENTRY_FOOTSTEPS).clamp(min=0)
    if _LADDER_DOUBLE_SUPPORT:
        target = target // 2
    return target.clamp(max=max(rung_count - 1, 0))


def _ladder_target_foot_id(
    target: torch.Tensor, rung_count: int
) -> torch.Tensor:
    """Return the left/right foot selected by a planned foothold index."""
    del rung_count
    return (target.long() + _LADDER_FIRST_SWING_FOOT) % 2


def _ladder_raw_rung_contact_flags(
    env: ManagerBasedRlEnv,
    rung_count: int,
) -> torch.Tensor | None:
    """Return real MJWarp foot-to-rung contacts as ``[env, rung, foot]``.

    The generic ``ContactSensor`` can filter one exact secondary geom, or no
    secondary geom at all.  A regex plus ``secondary_policy='any'`` therefore
    does *not* mean "any geom matching this regex"; it means "any geom".  The
    old per-rung sensors were configured that way and could report a floor or
    rail contact as a rung contact.  V3 progress must use the raw MJWarp
    contact table instead, where both geom ids and the owning vectorized world
    are available.
    """
    ...
    cache = getattr(env, "_ladder_raw_contact_geom_maps", None)
    if cache is None:
        model = sim.mj_model
        foot_map = torch.full(
            (model.ngeom,), -1, dtype=torch.long, device=env.device
        )
        rung_map = torch.full(
            (model.ngeom,), -1, dtype=torch.long, device=env.device
        )
        for geom_id in range(model.ngeom):
            ...
            if short_name == "left_foot_collision":
                foot_map[geom_id] = 0
            elif short_name == "right_foot_collision":
                foot_map[geom_id] = 1
            marker = "_rung_"
            if marker in name:
                suffix = name.rsplit(marker, 1)[-1]
                if suffix.isdigit() and int(suffix) < rung_count:
                    rung_map[geom_id] = int(suffix)
        cache = (foot_map, rung_map)
        env._ladder_raw_contact_geom_maps = cache

    foot_map, rung_map = cache
    nacon = getattr(data, "nacon", None)
    if nacon is None:
        return None
    n_contacts = int(nacon[0].item())
    flags = torch.zeros(
        (env.num_envs, rung_count, 2), dtype=torch.bool, device=env.device
    )
    if n_contacts <= 0:
        return flags

    geom = contact.geom[:n_contacts].long()
    world_id = contact.worldid[:n_contacts].long()
    geom_a = geom[:, 0]
    geom_b = geom[:, 1]
    foot_a = foot_map[geom_a]
    foot_b = foot_map[geom_b]
    rung_a = rung_map[geom_a]
    rung_b = rung_map[geom_b]

    # MJWarp can retain sensor-only records in the contact table.  Only the
    # constraint bit represents a physical solver contact.
    contact_type = getattr(contact, "type", None)
    solver_contact = torch.ones_like(world_id, dtype=torch.bool)
    if contact_type is not None:
        solver_contact = (contact_type[:n_contacts].long() & 1) != 0

    pair_a = (foot_a >= 0) & (rung_b >= 0) & solver_contact
    pair_b = (foot_b >= 0) & (rung_a >= 0) & solver_contact
    if pair_a.any():
        flags[world_id[pair_a], rung_b[pair_a], foot_a[pair_a]] = True
    if pair_b.any():
        flags[world_id[pair_b], rung_a[pair_b], foot_b[pair_b]] = True
    return flags


def _ladder_raw_entry_contact_flags(
    env: ManagerBasedRlEnv,
) -> torch.Tensor | None:
    """Return real foot contacts with the raised lower entry timber."""
    return _ladder_raw_foot_contact_flags_with_markers(env, ("_floor_foot",))


def _ladder_raw_trunk_hard_contact_flags(
    env: ManagerBasedRlEnv,
) -> torch.Tensor | None:
    """Return trunk contacts with ladder geoms that are hard failures.

    ``ContactSensorCfg.secondary_policy='any'`` deliberately removes the
    secondary filter in mjlab; a regex there does not mean "any matching
    geom". The V3 task therefore classifies the vectorized MJWarp contact
    table directly. Crossbar brushes remain physical contacts and are
    penalized, while side rails, the entry timber, and the table assembly are
    the contacts that can terminate a climb.
    """
    sim = getattr(env, "sim", None)
    data = getattr(sim, "data", None)
    contact = getattr(data, "contact", None)
    ...
    cache = getattr(env, "_ladder_trunk_hard_geom_maps", None)
    if cache is None:
        model = sim.mj_model
        trunk_map = torch.zeros(model.ngeom, dtype=torch.bool, device=env.device)
        hard_map = torch.zeros(model.ngeom, dtype=torch.bool, device=env.device)
        for geom_id in range(model.ngeom):
            name = mujoco.mj_id2name(
                model, mujoco.mjtObj.mjOBJ_GEOM, geom_id
            )
            if not name:
                continue
            short_name = name.rsplit("/", 1)[-1]
            trunk_map[geom_id] = short_name == "trunk_body_collision"
            is_ladder_geom = short_name.startswith("video_ladder_")
            hard_suffix = (
                short_name.endswith("_left_rail")
                or short_name.endswith("_right_rail")
                or short_name.endswith("_floor_foot")
                or short_name.endswith("_table_top")
                or "_table_leg_" in short_name
                or "_top_landing_" in short_name
            )
            hard_map[geom_id] = is_ladder_geom and hard_suffix
        cache = (trunk_map, hard_map)
        env._ladder_trunk_hard_geom_maps = cache

    trunk_map, hard_map = cache
    nacon = getattr(data, "nacon", None)
    if nacon is None:
        return None
    n_contacts = int(nacon[0].item())
    ...
    geom_a = geom[:, 0]
    geom_b = geom[:, 1]
    trunk_a = trunk_map[geom_a]
    trunk_b = trunk_map[geom_b]
    hard_a = hard_map[geom_a]
    hard_b = hard_map[geom_b]
    contact_type = getattr(contact, "type", None)
    solver_contact = torch.ones_like(world_id, dtype=torch.bool)
    if contact_type is not None:
        solver_contact = (contact_type[:n_contacts].long() & 1) != 0

    pair_a = trunk_a & hard_b & solver_contact
    pair_b = trunk_b & hard_a & solver_contact
    if pair_a.any():
        flags[world_id[pair_a]] = True
    if pair_b.any():
        flags[world_id[pair_b]] = True
    return flags


def _ladder_rung_foot_contact_flags(
    env: ManagerBasedRlEnv,
    rung_count: int,
    sensor_name: str,
) -> torch.Tensor:
    """Return per-foot contact with any real rung as ``[env, foot]``.

    The raw table is preferred because the aggregate contact sensor cannot
    express a regex over all vectorized rung geoms.  The sensor fallback keeps
    unit-test and older mjlab environments usable.
    """
    raw_flags = _ladder_raw_rung_contact_flags(env, rung_count)
    if raw_flags is not None:
        return raw_flags.any(dim=1).float()
    ...
    return (found[:, :2] > 0).float()


def _ladder_target_contact_state(
    env: ManagerBasedRlEnv,
    target: torch.Tensor,
    rung_count: int,
    sensor_name: str,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return target-rung contact and its measured hold time.

    ``target`` selects both the rung and the alternating swing foot.  When
    running with MJWarp, the hold time comes from the in-environment clock fed
    by the raw foot-to-rung contact table; it is never inferred from a nearby
    foot pose.
    """
    target_index = _ladder_target_rung_index(target, rung_count)
    foot_id = _ladder_target_foot_id(target, rung_count)
    env_ids = torch.arange(env.num_envs, device=env.device)

    raw_flags = _ladder_raw_rung_contact_flags(env, rung_count)
    if raw_flags is not None:
        entry_target = _ladder_entry_target_mask(target)
        entry_flags = _ladder_raw_entry_contact_flags(env)
        if entry_flags is None:
            entry_flags = torch.zeros(
                (env.num_envs, 2), dtype=torch.bool, device=env.device
            )
        rung_contact = raw_flags[env_ids, target_index, foot_id]
        entry_contact = entry_flags[env_ids, foot_id]
        contact = torch.where(entry_target, entry_contact, rung_contact)
        contact_time = getattr(env, "_ladder_raw_contact_time", None)
        expected_shape = (env.num_envs, rung_count, 2)
        if contact_time is None or contact_time.shape != expected_shape:
            contact_time = torch.zeros(
                expected_shape, dtype=torch.float32, device=env.device
            )
        entry_time = getattr(env, "_ladder_raw_entry_contact_time", None)
        if entry_time is None or entry_time.shape != (env.num_envs, 2):
            entry_time = torch.zeros(
                (env.num_envs, 2), dtype=torch.float32, device=env.device
            )
        selected_rung_time = contact_time[env_ids, target_index, foot_id]
        selected_entry_time = entry_time[env_ids, foot_id]
        return contact, torch.where(
            entry_target, selected_entry_time, selected_rung_time
        )
    ...
    if found.dim() == 3:
        found = found.any(dim=-1)
    contact = found.gather(1, foot_id[:, None]).squeeze(1) > 0
    contact_time = sensor.data.current_contact_time
    if contact_time is None:
        contact_time = torch.where(
            contact,
            torch.full_like(found, float(env.step_dt)),
            torch.zeros_like(found),
        )
    return contact, contact_time.gather(1, foot_id[:, None]).squeeze(1)


def _ladder_raw_foot_contact_flags_with_markers(
    env: ManagerBasedRlEnv,
    markers: tuple[str, ...],
) -> torch.Tensor | None:
    """Return raw foot contacts against geoms whose short name has a marker."""
    sim = getattr(env, "sim", None)
    data = getattr(sim, "data", None)
    contact = getattr(data, "contact", None)
    if contact is None or not hasattr(contact, "geom"):
    ...
    cache_key = "_ladder_raw_marker_geom_maps_" + "|".join(markers)
    ...
    if cache is None:
        ...
        marker_map = torch.zeros(
            (model.ngeom,), dtype=torch.bool, device=env.device
        )
        for geom_id in range(model.ngeom):
            name = mujoco.mj_id2name(
                model, mujoco.mjtObj.mjOBJ_GEOM, geom_id
            ...
            if short_name == "left_foot_collision":
                ...
                foot_map[geom_id] = 1
            marker_map[geom_id] = any(marker in short_name for marker in markers)
        cache = (foot_map, marker_map)
        setattr(env, cache_key, cache)

    foot_map, marker_map = cache
    nacon = getattr(data, "nacon", None)
    if nacon is None:
        return None
    n_contacts = int(nacon[0].item())
    flags = torch.zeros((env.num_envs, 2), dtype=torch.bool, device=env.device)
    if n_contacts <= 0:
        return flags
    ...
    foot_a = foot_map[geom_a]
    foot_b = foot_map[geom_b]
    marked_a = marker_map[geom_a]
    marked_b = marker_map[geom_b]
    contact_type = getattr(contact, "type", None)
    solver_contact = torch.ones_like(world_id, dtype=torch.bool)
    if contact_type is not None:
        solver_contact = (contact_type[:n_contacts].long() & 1) != 0

    pair_a = (foot_a >= 0) & marked_b & solver_contact
    pair_b = (foot_b >= 0) & marked_a & solver_contact
    if pair_a.any():
        flags[world_id[pair_a], foot_a[pair_a]] = True
    if pair_b.any():
        flags[world_id[pair_b], foot_b[pair_b]] = True
    return flags


def _ladder_raw_table_foot_contact_flags(
    env: ManagerBasedRlEnv,
) -> torch.Tensor | None:
    """Return physical foot contact with the table or top landing blocks."""
    return _ladder_raw_foot_contact_flags_with_markers(
        env, ("_table_top", "_top_landing_")
    )


def _ladder_ordered_site_ids(
    env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg
) -> torch.Tensor:
    """Resolve ladder feet in the same order as the contact sensor.

    ``SceneEntityCfg.site_ids`` is not guaranteed to preserve the order of
    ``site_names`` after regex resolution.  The contact sensor, however, uses
    the explicit left/right primary-geom order.  A mismatched order makes the
    state machine combine one foot's contact time with the other foot's pose.
    """
    cache = getattr(env, "_ladder_ordered_site_ids", None)
    if cache is None or cache.device != env.device:
        asset = env.scene[asset_cfg.name]
        site_ids, _ = asset.find_sites(
            ("left_foot", "right_foot"), preserve_order=True
        )
        if not torch.is_tensor(site_ids):
            site_ids = torch.tensor(site_ids, dtype=torch.long, device=env.device)
        else:
            site_ids = site_ids.to(device=env.device, dtype=torch.long)
        env._ladder_ordered_site_ids = site_ids
        cache = site_ids
    return cache


def ladder_bootstrap_reset(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor,
    state_path: str,
    target_rung: int = 1,
    velocity_scale: float = 0.0,
    joint_velocity_scale: float | None = None,
) -> None:
    """Reset a bridge-curriculum batch from a real first-rung contact state.

    The state is captured from an actual MuJoCo rollout after the previous
    rung's contact/hold gate fires.  It is a training initializer, not a
    success shortcut: the next rung remains uncontacted and the normal
    contact-gated state machine must still advance.
    """
    if env_ids is None or len(env_ids) == 0:
        return
    cached = getattr(env, "_ladder_bootstrap_state", None)
    if cached is None:
        payload = torch.load(state_path, map_location=env.device, weights_only=True)
        cached = (
            torch.as_tensor(payload["qpos"], device=env.device),
            torch.as_tensor(payload["qvel"], device=env.device),
        )
        cached_action = payload.get("action")
        if cached_action is not None:
            cached = (*cached, torch.as_tensor(cached_action, device=env.device))
        env._ladder_bootstrap_state = cached
    qpos, qvel = cached[:2]
    env_ids = env_ids.to(device=env.device, dtype=torch.long)
    qpos_batch = qpos.unsqueeze(0).expand(len(env_ids), -1).clone()
    qvel_batch = qvel.unsqueeze(0).expand(len(env_ids), -1).clone()
    # The captured state is expressed in env 0's local frame.  Each vectorized
    # copy receives the same relative state at its own terrain origin.
    qpos_batch[:, :3] += env.scene.terrain.env_origins[env_ids]
    # The free joint velocity is a reset perturbation, while joint velocities
    # carry useful actuator/support information from the real contact event.
    # Keep both scales explicit: zeroing all velocities can create a solver
    # impulse that knocks the support foot off the rung, while keeping the
    # captured swing velocity unchanged is too aggressive for PPO.
    if joint_velocity_scale is None:
        joint_velocity_scale = velocity_scale
    qvel_batch[:, :6] *= float(velocity_scale)
    qvel_batch[:, 6:] *= float(joint_velocity_scale)
    env.sim.data.qpos[env_ids] = qpos_batch
    env.sim.data.qvel[env_ids] = qvel_batch
    if len(cached) >= 3:
        action_batch = cached[2].unsqueeze(0).expand(len(env_ids), -1)
        # ManagerBasedRlEnv resets the ActionManager after all reset events.
        # Keep the command pending so LadderBootstrapJointPositionAction can
        # restore it from its own reset hook instead of losing it here.
        pending = getattr(env, "_ladder_bootstrap_action", None)
        if pending is None or pending.shape != env.action_manager.action.shape:
            pending = torch.zeros_like(env.action_manager.action)
        pending[env_ids] = action_batch.to(dtype=pending.dtype)
        env._ladder_bootstrap_action = pending
    env._ladder_bootstrap_target_rung = int(target_rung)


def _ladder_active_rungs(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
) -> torch.Tensor:
    """Return the number of currently trainable rungs for each environment.

    The geometry stays discrete at every stage.  The schedule only changes the
    target horizon, so PPO first learns balance and one-step support before it
    has to solve the full fourteen-rung transfer.
    """
    plan_count = _ladder_plan_count(rung_count)
    active = int(stage_schedule[0][1]) if stage_schedule else plan_count
    # A bounded override is useful for short local stage smoke/training runs:
    # it does not change the production curriculum, but avoids spending the
    # first 4,800 global steps just to inspect the first physical rung.  The
    # variable is intentionally opt-in and is never set by normal commands.
    stage_override = os.environ.get("MICRODUCK_LADDER_ACTIVE_RUNGS")
    if stage_override is not None:
        try:
            active = max(0, min(int(stage_override), plan_count))
        except ValueError as exc:
            raise ValueError(
                "MICRODUCK_LADDER_ACTIVE_RUNGS must be an integer"
            ) from exc
        return torch.full(
            (env.num_envs,), active, dtype=torch.long, device=env.device
        )
    step = int(getattr(env, "common_step_counter", 0))
    for threshold, count in stage_schedule:
        if step >= int(threshold):
            active = int(count)
    active = max(0, min(active, plan_count))
    return torch.full(
        (env.num_envs,), active, dtype=torch.long, device=env.device
    )


def ladder_stage_curriculum(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
) -> torch.Tensor:
    """Expose the active foothold horizon as a curriculum metric."""
    del env_ids
    return _ladder_active_rungs(env, rung_count, stage_schedule).float().mean()
...
def ladder_domain_randomization(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor,
    start_step: int,
    foot_friction_range: tuple[float, float] = (0.85, 1.25),
    joint_friction_range: tuple[float, float] = (0.90, 1.10),
    mass_inertia_range: tuple[float, float] = (0.95, 1.05),
    kp_range: tuple[float, float] = (0.90, 1.10),
    kd_range: tuple[float, float] = (0.90, 1.10),
    ...
    mass_asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> None:
    """Enable sim-to-real perturbations only after nominal climbing is learned.

    This is a reset event rather than a reward-side shortcut.  Before
    ``start_step`` it is a no-op, so stages A--F see deterministic nominal
    contact and dynamics.  Afterward each reset samples physical variations
    from their nominal values, including the BAM actuator model and a small
    initial attitude perturbation.
    """
    if int(getattr(env, "common_step_counter", 0)) < int(start_step):
        return
    ...
    randomize_bam_friction(
        env,
        env_ids,
        scale_range=joint_friction_range,
    ...
    # Use mjlab's pseudo-inertia path for vectorized environments.  Directly
    # writing body_mass/body_inertia only changes the shared model in some
    # mjlab versions, so it is not a reliable per-environment perturbation.
    mass_lo, mass_hi = mass_inertia_range
    dr.pseudo_inertia(
        env,
        env_ids,
        alpha_range=(math.log(mass_lo) / 2.0, math.log(mass_hi) / 2.0),
        asset_cfg=mass_asset_cfg,
    )
...
def _ladder_rung_fraction(
    rung_index: torch.Tensor, rung_count: int
) -> torch.Tensor:
    """Match the scene's first/last rung margins for tensor indices."""
    # Keep the planner's target points numerically identical to the physical
    # MjSpec geometry.  The first rung is at the ladder origin; the last rung
    # keeps the small top margin used by the reference scene.
    return 0.0 + 0.95 * rung_index.float() / max(rung_count - 1, 1)


def _ladder_body_yaw(asset: Entity) -> torch.Tensor:
    quat = asset.data.root_link_quat_w
    w, x, y, z = quat.unbind(dim=-1)
    return torch.atan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))


def _ladder_world_to_body(delta_w: torch.Tensor, yaw: torch.Tensor) -> torch.Tensor:
    expand_shape = (yaw.shape[0],) + (1,) * (delta_w.ndim - 1)
    cos_yaw = torch.cos(yaw).reshape(expand_shape)
    sin_yaw = torch.sin(yaw).reshape(expand_shape)
    x_body = cos_yaw * delta_w[..., 0:1] + sin_yaw * delta_w[..., 1:2]
    y_body = -sin_yaw * delta_w[..., 0:1] + cos_yaw * delta_w[..., 1:2]
    return torch.cat((x_body, y_body, delta_w[..., 2:3]), dim=-1)


def _ladder_target_world(
    env: ManagerBasedRlEnv,
    rung_index: torch.Tensor,
    rung_count: int,
    start_x: float,
    start_z: float,
    finish_x: float,
    finish_z: float,
    foot_lateral: float,
    rung_surface_offset: float,
) -> torch.Tensor:
    origin = env.scene.terrain.env_origins
    entry_target = _ladder_entry_target_mask(rung_index)
    physical_rung = _ladder_target_rung_index(rung_index, rung_count)
    fraction = _ladder_rung_fraction(physical_rung, rung_count)
    rung_x = (
        start_x
        + fraction * (finish_x - start_x)
        + _LADDER_FOOT_SITE_X_OFFSET
    )
    rung_z = start_z + fraction * (finish_z - start_z) + rung_surface_offset
    # The floor foot is the physical first foothold.  Its top is at
    # start_z - 0.010 m in the scene; the target site is placed on that
    # surface so the collision geometry, not the site marker, determines the
    # actual contact.
    entry_x = torch.full_like(rung_x, start_x + _LADDER_ENTRY_FOOT_SITE_X_OFFSET)
    entry_z = torch.full_like(
        rung_z, start_z + _LADDER_ENTRY_FOOT_SITE_Z_OFFSET
    )
    x = torch.where(entry_target, entry_x, rung_x)
    z = torch.where(entry_target, entry_z, rung_z)
    # The XML's left leg is on +Y; alternate the target laterals to match the
    # physical left/right support sequence rather than asking both feet to use
    # the centre line of the ladder.
    foot_id = _ladder_target_foot_id(rung_index, rung_count)
    y = torch.where(
        foot_id == 0,
        torch.full_like(x, foot_lateral),
        torch.full_like(x, -foot_lateral),
    )
    return origin + torch.stack((x, y, z), dim=-1)


def _ladder_foothold_errors(
    env: ManagerBasedRlEnv,
    foot_pos: torch.Tensor,
    target_pos: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Return along-ladder, lateral, and height errors for a foot.

    A rung is a wide physical surface, not a point target.  The old check
    used one Euclidean distance to the alternating target center, so a valid
    contact near either edge of a rung could be rejected solely because of
    its lateral offset.  Keep the x/z alignment tight while measuring the
    lateral coordinate against the actual ladder centerline.
    """
    delta = foot_pos - target_pos
    along_height = torch.linalg.vector_norm(delta[..., (0, 2)], dim=-1)
    ladder_center_y = env.scene.terrain.env_origins[:, 1]
    lateral = torch.abs(foot_pos[..., 1] - ladder_center_y)
    target_lateral = torch.abs(delta[..., 1])
    return along_height, lateral, target_lateral
...
def _ladder_state(
    ...
    finish_x: float,
    finish_z: float,
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    target_tolerance: float = 0.085,
    hold_time: float = 0.20,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Update and return the physical foothold state machine.

    A target advances only after the selected foot has an actual MuJoCo
    contact and remains close to that rung for ``hold_time``.  Root x/z alone
    can therefore never advance the planner.
    """
    active = _ladder_active_rungs(env, rung_count, stage_schedule)
    target = getattr(env, "_ladder_target_rung", None)
    phase_time = getattr(env, "_ladder_phase_time", None)
    if target is None or target.shape != (env.num_envs,):
        target = torch.zeros(env.num_envs, dtype=torch.long, device=env.device)
        phase_time = torch.zeros(env.num_envs, device=env.device)
        env._ladder_target_rung = target
        env._ladder_phase_time = phase_time

    reset = env.episode_length_buf == 0
    bootstrap_target = int(getattr(env, "_ladder_bootstrap_target_rung", 0))
    plan_count = _ladder_plan_count(rung_count)
    target[reset] = min(max(bootstrap_target, 0), plan_count)
    phase_time[reset] = 0.0
    target.clamp_(min=0, max=plan_count)

    step = int(getattr(env, "common_step_counter", 0))
    marker = getattr(env, "_ladder_state_step", None)
    advanced = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    if marker != step:
        phase_time += float(env.step_dt)
        foot_id = _ladder_target_foot_id(target, rung_count)
        target_index = _ladder_target_rung_index(target, rung_count)

        # The raw MJWarp table is the authoritative per-rung signal.  Keep a
        # small in-environment contact clock because the raw table only stores
        # the current solver contacts, not a per-pair hold duration.
        raw_flags = _ladder_raw_rung_contact_flags(env, rung_count)
        if raw_flags is not None:
            exact_time = getattr(env, "_ladder_raw_contact_time", None)
            expected_shape = (env.num_envs, rung_count, 2)
            if exact_time is None or exact_time.shape != expected_shape:
                exact_time = torch.zeros(
                    expected_shape, dtype=torch.float32, device=env.device
                )
            exact_time[reset] = 0.0
            exact_time = torch.where(
                raw_flags,
                exact_time + float(env.step_dt),
                torch.zeros_like(exact_time),
            )
            env._ladder_raw_contact_time = exact_time
            entry_flags = _ladder_raw_entry_contact_flags(env)
            if entry_flags is None:
                entry_flags = torch.zeros(
                    (env.num_envs, 2), dtype=torch.bool, device=env.device
            ...
            entry_time[reset] = 0.0
            entry_time = torch.where(
                entry_flags,
                entry_time + float(env.step_dt),
                torch.zeros_like(entry_time),
            )
            env._ladder_raw_entry_contact_time = entry_time
            env_ids = torch.arange(env.num_envs, device=env.device)
            entry_target = _ladder_entry_target_mask(target)
            rung_found = raw_flags[env_ids, target_index, foot_id]
            entry_found = entry_flags[env_ids, foot_id]
            selected_found = torch.where(entry_target, entry_found, rung_found)
            rung_time = exact_time[env_ids, target_index, foot_id]
            selected_entry_time = entry_time[env_ids, foot_id]
            selected_time = torch.where(
                entry_target, selected_entry_time, rung_time
            )
        else:
            # CPU/unit-test fallback for older environments without MJWarp
            # contact storage.  This path is deliberately not used by the V3
            ...
            if sensor is not None:
                ...
                selected_found = (
                    found.gather(1, foot_id[:, None]).squeeze(1) > 0
                )
                selected_time = contact_time.gather(
                    1, foot_id[:, None]
                ).squeeze(1)
            else:
                selected_found = torch.zeros(
                    env.num_envs, dtype=torch.bool, device=env.device
                )
                selected_time = torch.zeros(
                    env.num_envs, dtype=torch.float32, device=env.device
                )

        if selected_found is not None:
            foot_pos = _ladder_ordered_site_ids(env, asset_cfg)
            asset = env.scene[asset_cfg.name]
            feet = asset.data.site_pos_w[:, foot_pos]
            selected_pos = feet.gather(
                1, foot_id[:, None, None].expand(-1, 1, 3)
            ).squeeze(1)
            target_pos = _ladder_target_world(
                env,
                target,
                rung_count,
                start_x,
            ...
            along_height, lateral, _ = _ladder_foothold_errors(
                env, selected_pos, target_pos
            )
            # Keep the last hard-gate inputs available to the physical audit
            # script without printing from the vectorized training loop.
            env._ladder_debug_active = active.clone()
            env._ladder_debug_selected_found = selected_found.clone()
            env._ladder_debug_selected_time = selected_time.clone()
            env._ladder_debug_along_height = along_height.clone()
            env._ladder_debug_lateral = lateral.clone()
            env._ladder_debug_selected_pos = selected_pos.clone()
            env._ladder_debug_target_pos = target_pos.clone()
            root_pos = asset.data.root_link_pos_w - env.scene.terrain.env_origins
            root_quat = asset.data.root_link_quat_w
            root_cos_tilt = 1.0 - 2.0 * (
                root_quat[:, 1].square() + root_quat[:, 2].square()
            )
            root_support_gap = root_pos[:, 2] - selected_pos[:, 2]
            root_speed = torch.linalg.vector_norm(
                asset.data.root_link_lin_vel_b[:, :2], dim=-1
            )
            env._ladder_debug_root_pos = root_pos.clone()
            env._ladder_debug_root_support_gap = root_support_gap.clone()
            env._ladder_debug_root_cos_tilt = root_cos_tilt.clone()
            env._ladder_debug_root_speed = root_speed.clone()
            # The first transfer starts from flat ground, so a fixed 5.5 cm
            # root-above-sole gate is too discontinuous for PPO: a valid foot
            # can hold the first low rung while the old support foot still
            # carries the COM.  Let the contact/hold/upright gate expose that
            # transition, then tighten the required trunk height as the
            # planner climbs.  The landing and post-landing rewards continue
            # to push every rung toward the final 5.5 cm support margin.
            ...
            advanced = (
                (target < active)
                & selected_found
                & (selected_time >= hold_time)
                # The rung has a 0.20 m half-width in the scene.  Requiring
                # contact plus x/z alignment and a centerline bound accepts
                # a real edge landing without accepting the floor or a rail.
                & (along_height <= target_tolerance)
                & (lateral <= 0.18)
                # A contact tap still cannot advance: the foot must be
                # spatially aligned, held, upright, and low-speed.  The
                # height gate is a progressive support-transfer curriculum.
                & (root_support_gap >= required_support_gap)
                & (root_cos_tilt >= math.cos(math.radians(42.0)))
                & (root_speed <= 0.65)
            )
            target += advanced.long()
            phase_time = torch.where(
                advanced, torch.zeros_like(phase_time), phase_time
            )
        env._ladder_state_step = step
        env._ladder_advanced_this_step = advanced

    advanced = getattr(
        env,
        "_ladder_advanced_this_step",
        torch.zeros(env.num_envs, dtype=torch.bool, device=env.device),
    )
    env._ladder_target_rung = target
    env._ladder_phase_time = phase_time
    return target, active, phase_time, advanced


def ladder_step_targets(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    finish_x: float,
    finish_z: float,
    sensor_name: str = "ladder_foot_contact",
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Observation: current and next foothold as [dx, dy, dz, yaw] × 2."""
    target, active, phase_time, _ = _ladder_state(
        env,
        rung_count,
        stage_schedule,
        sensor_name,
    ...
    asset = env.scene[asset_cfg.name]
    root = asset.data.root_link_pos_w
    yaw = _ladder_body_yaw(asset)
    poses = []
    for offset in (0, 1):
        index = (target + offset).clamp(
            max=_ladder_plan_count(rung_count) - 1
        )
        goal = _ladder_target_world(
            env,
            index,
            rung_count,
            start_x,
        ...
        relative = _ladder_world_to_body(goal - root, yaw)
        target_yaw = wrap_to_pi(-yaw)
        poses.append(torch.cat((relative, target_yaw[:, None]), dim=-1))
    result = torch.cat(poses, dim=-1)
    return torch.where((active > 0)[:, None], result, torch.zeros_like(result))


def ladder_phase_observation(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    finish_x: float = 0.72,
    finish_z: float = 0.67,
    cycle_time: float = 1.60,
) -> torch.Tensor:
    _, active, phase_time, _ = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        start_x, start_z, finish_x, finish_z,
    )
    cycle_time = max(cycle_time, 1e-4)
    phase = 2.0 * math.pi * torch.remainder(phase_time, cycle_time) / cycle_time
    result = torch.stack((torch.sin(phase), torch.cos(phase)), dim=-1)
    return torch.where((active > 0)[:, None], result, torch.zeros_like(result))


def ladder_swing_foot_observation(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    target, active, _, _ = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        start_x, start_z, finish_x, finish_z,
    )
    foot = _ladder_target_foot_id(target, rung_count)
    one_hot = torch.nn.functional.one_hot(foot, num_classes=2).float()
    return torch.where((active > 0)[:, None], one_hot, torch.zeros_like(one_hot))


def ladder_foot_positions_observation(
    env: ManagerBasedRlEnv,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    asset = env.scene[asset_cfg.name]
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    root = asset.data.root_link_pos_w[:, None, :]
    yaw = _ladder_body_yaw(asset)
    relative = _ladder_world_to_body_yaw_safe(feet - root, yaw)
    return relative.reshape(env.num_envs, -1)


def _ladder_world_to_body_yaw_safe(
    delta_w: torch.Tensor, yaw: torch.Tensor
) -> torch.Tensor:
    return _ladder_world_to_body(delta_w, yaw)


def ladder_foot_contacts_observation(
    env: ManagerBasedRlEnv,
    sensor_name: str = "ladder_foot_contact",
    rung_count: int = 14,
) -> torch.Tensor:
    ...
    if raw_flags is not None:
        ...
        return (raw_flags.any(dim=1) | entry_flags).float()
    sensor = env.scene.sensors.get(sensor_name)
    if sensor is None:
        return torch.zeros((env.num_envs, 2), device=env.device)
    found = sensor.data.found
...
def ladder_support_state_observation(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Expose the physical support-transfer state to the policy.

    Foot positions and rung targets are already part of V3, but the policy
    otherwise has to learn the useful combination ``root_z - support_foot_z``
    from several separately normalized terms.  That is especially sparse at
    the first rung: the old warm start learned to tap the rung while keeping
    the pelvis below it.  These six values make the missing state explicit
    without exposing contact IDs or bypassing MuJoCo contact sensing:

    ``contact, support_gap, root_dx, root_dz, upright, planar_speed``.
    All values are bounded and expressed relative to the current planned rung.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    foot_id = _ladder_target_foot_id(target, rung_count)
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    selected = feet.gather(
        1, foot_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)
    goal = _ladder_target_world(
        env,
        target.clamp(max=_ladder_plan_count(rung_count) - 1),
        rung_count,
        start_x,
        start_z,
        finish_x,
    ...
    root = asset.data.root_link_pos_w - env.scene.terrain.env_origins
    root_goal = goal.clone()
    root_goal[:, 0] -= 0.03
    root_goal[:, 2] += 0.07

    contact, _ = _ladder_target_contact_state(
        env, target, rung_count, sensor_name
    )
    contact = contact.float()

    quat = asset.data.root_link_quat_w
    upright = 1.0 - 2.0 * (quat[:, 1].square() + quat[:, 2].square())
    speed = torch.linalg.vector_norm(asset.data.root_link_lin_vel_b[:, :2], dim=-1)
    result = torch.stack(
        (
            contact,
            torch.clamp((root[:, 2] - selected[:, 2]) / 0.12, -1.0, 1.0),
            torch.clamp((root_goal[:, 0] - root[:, 0]) / 0.20, -1.0, 1.0),
            torch.clamp((root_goal[:, 2] - root[:, 2]) / 0.16, -1.0, 1.0),
            torch.clamp(upright, -1.0, 1.0),
            torch.clamp(speed / 0.8, 0.0, 1.0),
        ),
        dim=-1,
    )
    return torch.where((active > 0)[:, None], result, torch.zeros_like(result))
...
def ladder_stage_observation(
    ...
    return torch.stack(
        (
            active.float() / max(plan_count, 1),
            target.float() / max(plan_count, 1),
        ),
        dim=-1,
    )


def ladder_target_foot_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    target, active, _, _ = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        start_x, start_z, finish_x, finish_z,
        foot_lateral, rung_surface_offset, asset_cfg=asset_cfg,
    )
    asset = env.scene[asset_cfg.name]
    foot_id = _ladder_target_foot_id(target, rung_count)
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    selected = feet.gather(1, foot_id[:, None, None].expand(-1, 1, 3)).squeeze(1)
    goal = _ladder_target_world(
        env, target.clamp(max=_ladder_plan_count(rung_count) - 1), rung_count,
        start_x, start_z, finish_x, finish_z,
        foot_lateral, rung_surface_offset,
    )
    along_height, lateral, target_lateral = _ladder_foothold_errors(
        env, selected, goal
    )
    error = torch.sqrt(
        along_height.square()
        + (0.45 * target_lateral).square()
        + 0.10 * lateral.square()
    )
    # Use a potential difference instead of paying a constant absolute score.
    # Otherwise a policy can stand still at the reset pose and collect a small
    # but persistent reward without ever bringing the swing foot to the rung.
    potential = torch.exp(-error / max(distance_scale, 1e-4))
    previous = getattr(env, "_ladder_target_foot_potential_prev", None)
    previous_target = getattr(env, "_ladder_target_foot_previous_target", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    if previous_target is None or previous_target.shape != target.shape:
        previous_target = target.clone()
    fresh = env.episode_length_buf <= 1
    target_changed = previous_target != target
    baseline = torch.where(fresh | target_changed, potential, previous)
    delta = potential - baseline
    env._ladder_target_foot_potential_prev = potential.clone()
    env._ladder_target_foot_previous_target = target.clone()
    active_mask = (target < active) & (active > 0)
    # Keep a small absolute foothold signal in addition to the potential
    # difference.  Planned-footstep policies need a usable gradient while
    # approaching a target; the state machine below still requires real
    # rung contact before any target can advance.
    # The absolute term is deliberately larger during the one-rung bootstrap.
    # With only a potential difference, a standing policy receives almost no
    # gradient before its first exploratory swing.  This term points the
    # selected foot at the planned rung; the state machine still requires
    # actual rung contact and a hold interval before progress is credited.
    # Once the first rung is secured, a large absolute residual rewards
    # standing still below the next rung.  Keep only a small exploration
    # floor; the potential increase from lifting and advancing the swing foot
    # must dominate.
    shaped = delta + 0.05 * potential
    return torch.where(active_mask, shaped, torch.zeros_like(shaped))


def ladder_target_contact_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    foot_id = _ladder_target_foot_id(target, rung_count)
    contact, contact_time = _ladder_target_contact_state(
        env, target, rung_count, sensor_name
    )
    contact = contact & (contact_time > 0.02)
    asset = env.scene[asset_cfg.name]
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    selected = feet.gather(1, foot_id[:, None, None].expand(-1, 1, 3)).squeeze(1)
    goal = _ladder_target_world(
    ...
    # A foot touching the floor or a side rail is not a successful rung
    # contact.  The actual contact sensor is still the source of truth; this
    # spatial gate only associates that contact with the planner's target.
    along_height, lateral, target_lateral = _ladder_foothold_errors(
        env, selected, goal
    )
    # Contact is still mandatory; this term only shapes the approach.  Use
    # the wide rung surface for the lateral gate and reserve the sharper
    # Gaussian for the along-ladder/height alignment.
    on_rung = (lateral <= 0.18).float()
    on_target = on_rung * torch.exp(
        -torch.square(along_height / 0.060)
        -torch.square(target_lateral / 0.14)
    )
    return (
        contact.float()
        * on_target
...
def ladder_landing_stability_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Reward settling the whole body after a real rung landing.

    A contact event alone is not a usable biped support state: the duck can
    briefly tap a rung with a raised foot while the trunk is still below it.
    This term supplies the missing dense signal for the transfer from contact
    to support.  It is intentionally gated by the selected foot's measured
    rung contact and never changes the foothold state machine.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    on_target = torch.exp(
        -torch.square(along_height / 0.065)
        -torch.square(target_lateral / 0.14)
    ) * (lateral <= 0.18).float()

    root = asset.data.root_link_pos_w - env.scene.terrain.env_origins
    # The root should rise above the contacted sole and remain just behind the
    # foot along the ladder.  These are relative targets, so they work for
    # every rung and do not award free root translation before contact.
    root_goal = goal.clone()
    root_goal[:, 0] -= 0.03
    root_goal[:, 2] += 0.07
    root_error = torch.sqrt(
        (root[:, 0] - root_goal[:, 0]).square()
        + (root[:, 2] - root_goal[:, 2]).square()
    )
    root_score = torch.exp(-root_error / 0.085)
    # The old Euclidean target was nearly flat in the useful region: a tap
    # with the pelvis below the rung and a true support transfer could receive
    # similar scores.  Give the policy a direct monotonic signal for the
    # missing physical transition while leaving the hard state-machine gate
    # unchanged.  At the observed tap (gap ~= -0.04 m) this is small but
    # non-zero; it becomes strong only once the root is above the sole.
    support_gap = root[:, 2] - selected[:, 2]
    gap_score = torch.sigmoid((support_gap - 0.015) / 0.03)
    quat = asset.data.root_link_quat_w
    upright = 1.0 - 2.0 * (quat[:, 1].square() + quat[:, 2].square())
    upright_score = torch.clamp(
        (upright - math.cos(math.radians(45.0))) / 0.25, 0.0, 1.0
    )
    root_speed = torch.linalg.vector_norm(
        asset.data.root_link_lin_vel_b[:, :2], dim=-1
    )
    settle_score = torch.exp(-root_speed.square() / 0.45**2)
    # A contact on the new rung is only the beginning of a biped transfer.
    # The previous support foot can keep the body perfectly stable while the
    # swing foot is parked on the crossbar.  In that state the vertical gap
    # and root pose terms alone do not tell PPO which way to move next.  Use
    # the measured COM projection to reward loading the newly contacted foot;
    # this remains contact-gated and cannot advance the foothold state machine.
    com_y = asset.data.root_com_pos_w[:, 1]
    transfer_score = torch.exp(
        -torch.square((com_y - selected[:, 1]) / 0.055)
    )
    return (
        contact.float()
        * on_target
...
def ladder_post_landing_transfer_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Guide COM transfer after a real target-rung contact.

    The previous support foot can keep a newly landed foot parked on a rung
    for several seconds without ever loading that rung.  The next foothold
    must still pass the hard contact/hold/root gate, but PPO needs a dense
    potential for the missing transition: move the measured COM toward the
    contacted foot and raise the trunk above its sole.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    root = asset.data.root_link_pos_w - env.scene.terrain.env_origins
    com_y = asset.data.root_com_pos_w[:, 1]
    com_potential = torch.exp(-torch.abs(com_y - selected[:, 1]) / 0.08)
    support_gap = root[:, 2] - selected[:, 2]
    gap_potential = torch.sigmoid((support_gap - 0.015) / 0.03)
    potential = 0.58 * com_potential + 0.42 * gap_potential

    previous = getattr(env, "_ladder_post_landing_potential_prev", None)
    previous_target = getattr(env, "_ladder_post_landing_previous_target", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    if previous_target is None or previous_target.shape != target.shape:
        previous_target = target.clone()
    ...
    baseline = torch.where(fresh | target_changed, potential, previous)
    delta = potential - baseline
    env._ladder_post_landing_potential_prev = potential.clone()
    env._ladder_post_landing_previous_target = target.clone()

    active_mask = (target < active) & (active > 0)
    return torch.where(
        contact & (on_target > 0.25) & active_mask,
        delta + 0.035 * potential,
        torch.zeros_like(potential),
    )


def ladder_body_alignment_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Shape whole-body approach toward the current planned foothold.

    This is a target-relative alignment term, not a free root-x/root-z
    progress reward.  It gives the policy a usable signal before either foot
    can reach the first rung, while the state machine still requires real
    contact and a hold interval before advancing.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    horizontal_error = torch.linalg.vector_norm(root[:, :2] - goal[:, :2], dim=-1)
    potential = torch.exp(-horizontal_error / max(distance_scale, 1e-4))

    # Use the target-relative potential difference rather than paying a
    # constant reward for standing still.  This is still not free root
    # progress: the goal is the currently planned rung, and the state machine
    # remains the only mechanism that can advance to the next rung.
    previous = getattr(env, "_ladder_body_alignment_potential_prev", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    fresh = env.episode_length_buf <= 1
    baseline = torch.where(fresh, potential, previous)
    delta = potential - baseline
    env._ladder_body_alignment_potential_prev = potential.clone()
    active_mask = (target < active) & (active > 0)
    # Keep a visible whole-body alignment signal during approach.  It is
    # target-relative rather than free root-x progress, so it cannot advance
    # the foothold state machine by itself.
    shaped = delta + 0.12 * potential
    return torch.where(active_mask, shaped, torch.zeros_like(shaped))


def ladder_heading_alignment_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    finish_z: float = 0.67,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
    error_scale: float = 0.35,
) -> torch.Tensor:
    """Keep the body facing the ladder while preserving foothold progress.

    The old bridge checkpoint could secure one rung while yawing away from the
    ladder.  A swing leg then had to cross both the rung height and a large
    lateral body error, which made the first transfer look like a fall rather
    than a planned step.  This term is a heading potential, not a root-motion
    shortcut: the rung state machine still requires real contact to advance.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    asset = env.scene[asset_cfg.name]
    yaw = _ladder_body_yaw(asset)
    score = torch.exp(-torch.square(wrap_to_pi(yaw)) / max(error_scale, 1e-4) ** 2)
    previous = getattr(env, "_ladder_heading_potential_prev", None)
    if previous is None or previous.shape != score.shape:
        previous = score.clone()
    fresh = env.episode_length_buf <= 1
    baseline = torch.where(fresh, score, previous)
    delta = score - baseline
    env._ladder_heading_potential_prev = score.clone()
    active_mask = (target < active) & (active > 0)
    return torch.where(active_mask, delta + 0.08 * score, torch.zeros_like(score))


def ladder_support_transfer_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    cycle_time: float = 1.90,
    pre_swing_time: float = 0.45,
    lateral_scale: float = 0.045,
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Teach the body to transfer over the support foot before swinging.

    A one-rung bridge can look successful while the other foot is still on the
    floor. Asking that foot to swing immediately makes the MicroDuck fall or
    push its body into the side rail. During the short pre-swing interval,
    reward only the lateral projection of the COM moving over the foot that is
    carrying the previous rung. This is shaping only: the contact and hold
    gate in ``_ladder_state`` remains the only way to advance the task.
    """
    target, active, phase_time, _ = _ladder_state(
        env,
        rung_count,
    ...
    asset = env.scene[asset_cfg.name]
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    swing_id = _ladder_target_foot_id(target, rung_count)
    support_id = (swing_id + 1) % 2
    support_pos = feet.gather(
        1, support_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)

    def _contact_flags(sensor) -> torch.Tensor:
        if sensor is None:
            return torch.zeros((env.num_envs, 2), device=env.device)
        found = sensor.data.found
        if found.dim() == 3:
    ...
    support_contact = torch.maximum(
        _ladder_rung_foot_contact_flags(env, rung_count, sensor_name),
        _contact_flags(env.scene.sensors.get(ground_sensor_name)),
    ).gather(1, support_id[:, None]).squeeze(1)

    # Constrain only the lateral transfer. The COM is allowed to stay behind
    # the rung in x, which avoids turning this term into a hidden position goal.
    com_y = asset.data.root_com_pos_w[:, 1]
    lateral_error = torch.abs(com_y - support_pos[:, 1])
    # Use an L1-style distance potential here.  The old squared Gaussian was
    # effectively zero for the bridge reset (the COM starts roughly 10 cm
    # from the rung foot while ``lateral_scale`` is 4.5 cm), so PPO received
    # no usable signal for the first, essential load transfer.  This remains
    # a bounded potential and is still gated by real support contact.
    score = torch.exp(-lateral_error / max(lateral_scale, 1e-4))
    potential = score * support_contact
    previous = getattr(env, "_ladder_support_transfer_potential_prev", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    fresh = env.episode_length_buf <= 1
    baseline = torch.where(fresh, potential, previous)
    delta = potential - baseline
    env._ladder_support_transfer_potential_prev = potential.clone()

    phase = torch.remainder(phase_time, max(cycle_time, 1e-4))
    in_transfer = phase < max(pre_swing_time, 0.0)
    active_mask = (target < active) & (active > 0)
    # Center the small residual so a stationary, poorly transferred bridge
    # state cannot farm a positive reward forever.
    contact_score = 2.0 * support_contact - 1.0
    shaped = (
        delta
        # Keep a small absolute residual during the short transfer window so
        # a far-away COM is not a flat zero-reward basin.  The contact-gated
        # potential difference remains the dominant term.
        + 0.08 * score * support_contact
        + 0.02 * contact_score
    )
    return torch.where(
        active_mask & in_transfer, shaped, torch.zeros_like(shaped)
    )


def ladder_support_pose_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    joint_scale: float = 0.18,
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Keep the loaded support leg stable while the COM is transferred.

    The support reference is captured at reset and refreshed only after a
    real rung transition. This lets the policy move the pelvis and swing leg,
    while the hip-pitch/knee/ankle chain that is already supporting the robot
    is kept near a physically valid contact pose. It is intentionally limited
    to the pre-swing interval and cannot advance the foothold state machine.
    """
    target, active, phase_time, advanced = _ladder_state(
        env,
        rung_count,
        stage_schedule,
        sensor_name,
    ...
    joint_pos = asset.data.joint_pos[:, joint_ids].reshape(env.num_envs, 2, 3)
    swing_id = _ladder_target_foot_id(target, rung_count)
    support_id = (swing_id + 1) % 2

    reference = getattr(env, "_ladder_support_pose_reference", None)
    reference_target = getattr(env, "_ladder_support_pose_target", None)
    if reference is None or reference.shape != joint_pos.shape:
        reference = joint_pos.clone()
        reference_target = target.clone()
    refresh = (env.episode_length_buf <= 1) | (reference_target != target)
    reference = torch.where(refresh[:, None, None], joint_pos, reference)
    env._ladder_support_pose_reference = reference.clone()
    env._ladder_support_pose_target = target.clone()

    selected_reference = reference.gather(
        1, support_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)
    selected_joints = joint_pos.gather(
        1, support_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)
    error = torch.linalg.vector_norm(
        (selected_joints - selected_reference) / max(joint_scale, 1e-4),
        dim=-1,
    )
    score = torch.exp(-0.5 * error.square())
    ...
    active_mask = (target < active) & (active > 0)
    return torch.where(
        active_mask & in_transfer,
        score * support_contact,
        torch.zeros_like(score),
    )


def ladder_joint_teacher_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    cycle_time: float = 1.90,
    pre_swing_time: float = 0.45,
    swing_time: float = 1.10,
    joint_scale: float = 0.32,
    joint_names: tuple[str, ...] = (
        "left_hip_pitch",
        "left_knee",
        "left_ankle",
    ...
    """Track an optional MuJoCo-solved swing-leg pose during bootstrap.

    The teacher is deliberately an action-space residual, not a reset or a
    foothold shortcut.  It only supplies a smooth joint target for the
    currently planned swing leg; the raw rung contact and hold gate remain
    the only mechanism that can advance the ladder state machine.  This is
    useful for the first sparse-contact stage, where a walking checkpoint can
    stay alive without ever discovering the torque direction needed to lift
    the foot.
    """
    if not state_path:
        return torch.zeros(env.num_envs, device=env.device)
    ...
    cached = getattr(env, "_ladder_joint_teacher_target", None)
    if cached is None or cached.device != env.device:
        payload = torch.load(state_path, map_location=env.device, weights_only=True)
        teacher = payload.get("joint_pos")
        if teacher is None:
            raise KeyError(f"teacher state has no joint_pos: {state_path}")
        teacher = torch.as_tensor(teacher, device=env.device, dtype=joint_pos.dtype)
        if teacher.numel() == len(asset.joint_names):
            teacher = teacher[joint_ids]
        elif teacher.numel() != len(joint_names):
            raise ValueError(
                f"teacher joint_pos has {teacher.numel()} values; "
                f"expected {len(joint_names)} or {len(asset.joint_names)}"
            )
        env._ladder_joint_teacher_target = teacher.reshape(2, 3)
        cached = env._ladder_joint_teacher_target

    reference = getattr(env, "_ladder_joint_teacher_reference", None)
    reference_target = getattr(env, "_ladder_joint_teacher_reference_target", None)
    if reference is None or reference.shape != joint_pos.shape:
        reference = joint_pos.clone()
        reference_target = target.clone()
    refresh = (env.episode_length_buf <= 1) | (reference_target != target)
    reference = torch.where(refresh[:, None, None], joint_pos, reference)
    env._ladder_joint_teacher_reference = reference.clone()
    env._ladder_joint_teacher_reference_target = target.clone()

    swing_id = _ladder_target_foot_id(target, rung_count)
    current = joint_pos.gather(
        1, swing_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)
    start = reference.gather(
        1, swing_id[:, None, None].expand(-1, 1, 3)
    ).squeeze(1)
    goal = cached[swing_id]
    phase = torch.remainder(phase_time, max(cycle_time, 1e-4))
    alpha = torch.clamp(
        (phase - max(pre_swing_time, 0.0)) / max(swing_time, 1e-4),
        0.0,
        1.0,
    )
    smooth = alpha * alpha * (3.0 - 2.0 * alpha)
    desired = start + smooth[:, None] * (goal - start)
    error = torch.linalg.vector_norm(
        (current - desired) / max(joint_scale, 1e-4), dim=-1
    )
    score = torch.exp(-0.5 * error.square())
    active_mask = (target < active) & (active > 0)
    return torch.where(active_mask, score, torch.zeros_like(score))


def ladder_root_teacher_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    pre_swing_time: float = 0.45,
    swing_time: float = 1.10,
    position_scale: float = 0.075,
    orientation_scale: float = 0.30,
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    """Track a solved whole-body support-transfer pose during stage B.

    A joint-only teacher can ask the swing leg to fold while the free base
    drops back toward the floor.  That leaves the foot short of the rung even
    when its height looks plausible.  This optional term tracks the local
    root position and upright quaternion from the MuJoCo-solved bridge pose,
    while the support contact, collision checks, and rung hold gate remain
    unchanged.  It is disabled by default and is intended only for the first
    physical transfer curriculum.
    """
    if not state_path:
        return torch.zeros(env.num_envs, device=env.device)
    target, active, phase_time, _ = _ladder_state(
    ...
    asset = env.scene[asset_cfg.name]
    cached = getattr(env, "_ladder_root_teacher_target", None)
    if cached is None or cached[0].device != env.device:
        payload = torch.load(state_path, map_location=env.device, weights_only=True)
        qpos = payload.get("qpos")
        if qpos is None or torch.as_tensor(qpos).numel() < 7:
            raise ValueError(f"teacher state has no free-joint qpos: {state_path}")
        qpos = torch.as_tensor(qpos, device=env.device, dtype=asset.data.root_link_pos_w.dtype)
        root_pos = qpos[:3]
        root_quat = qpos[3:7]
        root_quat = root_quat / torch.linalg.vector_norm(root_quat).clamp_min(1e-6)
        env._ladder_root_teacher_target = (root_pos, root_quat)
        cached = env._ladder_root_teacher_target
    root_target, quat_target = cached

    root_pos = asset.data.root_link_pos_w - env.scene.terrain.env_origins
    root_quat = asset.data.root_link_quat_w
    root_quat = root_quat / torch.linalg.vector_norm(root_quat, dim=-1, keepdim=True).clamp_min(1e-6)
    root_error = torch.linalg.vector_norm(root_pos - root_target, dim=-1)
    quat_dot = torch.abs(torch.sum(root_quat * quat_target, dim=-1)).clamp(0.0, 1.0)
    orientation_error = 2.0 * torch.acos(quat_dot)
    score = torch.exp(
        -0.5 * (root_error / max(position_scale, 1e-4)).square()
        -0.5 * (orientation_error / max(orientation_scale, 1e-4)).square()
    )

    phase = torch.remainder(phase_time, max(cycle_time, 1e-4))
    active_phase = phase < max(pre_swing_time + swing_time, 0.0)
    active_mask = (target < active) & (active > 0) & active_phase
    return torch.where(active_mask, score, torch.zeros_like(score))


def ladder_step_advance_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    _, active, _, advanced = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        start_x, start_z, finish_x, finish_z,
    )
    return advanced.float() * (active > 0).float()


def ladder_support_stability_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    support_target = (target - 1).clamp(min=0)
    support_id = _ladder_target_foot_id(support_target, rung_count)
    rung_support = _ladder_rung_foot_contact_flags(
        env, rung_count, sensor_name
    ).gather(1, support_id[:, None]).squeeze(1) > 0
    entry_support = _ladder_raw_entry_contact_flags(env)
    if entry_support is None:
        entry_support = torch.zeros(
            (env.num_envs, 2), dtype=torch.bool, device=env.device
        )
    support = torch.where(
        _ladder_entry_target_mask(support_target),
        entry_support.gather(1, support_id[:, None]).squeeze(1),
        rung_support,
    )
    asset = env.scene[asset_cfg.name]
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    ...
    support_goal = _ladder_target_world(
        env,
        support_target.clamp(max=_ladder_plan_count(rung_count) - 1),
        rung_count,
        start_x,
        start_z,
        finish_x,
    ...
    support_on_target = (
        torch.linalg.vector_norm(support_pos - support_goal, dim=-1)
        <= support_tolerance
    )
    stable = torch.exp(-torch.square(asset.data.root_link_lin_vel_b[:, :2]).sum(dim=-1) / 0.08**2)
    return support.float() * support_on_target.float() * (target > 0).float() * stable


def ladder_swing_clearance_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    foot_id = _ladder_target_foot_id(target, rung_count)
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    selected_z = feet[:, :, 2].gather(1, foot_id[:, None]).squeeze(1)
    goal = _ladder_target_world(
        env, target.clamp(max=_ladder_plan_count(rung_count) - 1), rung_count,
        start_x,
        start_z,
        ...
        finish_z,
        0.065,
        _LADDER_FOOT_SITE_Z_OFFSET,
    )
    phase = torch.remainder(phase_time, max(cycle_time, 1e-4))
    swing_alpha = torch.clamp(
        (phase - max(pre_swing_time, 0.0)) / max(swing_time, 1e-4),
        0.0,
        1.0,
    )
    smooth = swing_alpha * swing_alpha * (3.0 - 2.0 * swing_alpha)
    in_swing = (phase >= max(pre_swing_time, 0.0)) & (
        phase < max(pre_swing_time, 0.0) + swing_time
    )
    # Ask the foot to clear the lower timber first, then converge to the
    # actual rung height.  A direct full-height target from the reset state is
    # too discontinuous for the short-legged MicroDuck and encourages a
    # floor-locked policy.  This is shaping only; MuJoCo contact still gates
    # foothold advancement.
    clearance_goal = torch.maximum(
        torch.full_like(goal[:, 2], 0.040),
        goal[:, 2] * smooth,
    )
    desired_z = torch.where(in_swing, clearance_goal, goal[:, 2])
    lift = torch.sigmoid((selected_z - desired_z + 0.012) / 0.012)
    return lift * (target < active).float() * (active > 0).float()


def ladder_footstep_trajectory_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    distance_scale: float = 0.075,
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Track a phase-conditioned swing arc toward the planned rung.

    The reference is generated from the current measured foot pose whenever a
    new target is exposed.  During swing it uses a smooth lift arc; during the
    settling interval it stays on the target pose.  This is a dense imitation
    signal for the planned-footstep method, not a success shortcut: only the
    MuJoCo rung contact and hold gate in ``_ladder_state`` can advance the
    foothold index.
    """
    if cycle_time <= 0.0 or not 0.0 < swing_time < cycle_time:
        raise ValueError("expected 0 < swing_time < cycle_time")
    target, active, phase_time, _ = _ladder_state(
        env,
        rung_count,
        stage_schedule,
    ...
    previous_target = getattr(env, "_ladder_reference_target", None)
    reference_start = getattr(env, "_ladder_reference_start", None)
    if previous_target is None or previous_target.shape != target.shape:
        previous_target = target.clone()
    if reference_start is None or reference_start.shape != selected.shape:
        reference_start = selected.clone()
    refresh = (env.episode_length_buf <= 1) | (previous_target != target)
    reference_start = torch.where(refresh[:, None], selected, reference_start)
    env._ladder_reference_target = target.clone()
    env._ladder_reference_start = reference_start.clone()

    phase = torch.remainder(phase_time, cycle_time)
    swing_phase = phase - max(pre_swing_time, 0.0)
    alpha = torch.clamp(swing_phase / swing_time, 0.0, 1.0)
    smooth = alpha * alpha * (3.0 - 2.0 * alpha)
    arc = reference_start + smooth[:, None] * (goal - reference_start)
    arc[:, 2] += lift_height * torch.sin(math.pi * alpha)
    in_swing = (phase >= max(pre_swing_time, 0.0)) & (
        phase < max(pre_swing_time, 0.0) + swing_time
    )
    desired = torch.where(in_swing[:, None], arc, goal)
    scaled_error = (selected - desired) / max(distance_scale, 1e-4)
    score = torch.exp(-0.5 * torch.sum(scaled_error.square(), dim=-1))
    # Center the tracking score.  A strictly positive absolute score lets a
    # stationary policy farm the easy beginning of every phase cycle; a foot
    # that does not follow the moving reference must pay a persistent cost.
    score = 2.0 * score - 1.0
    active_mask = (target < active) & (active > 0)
    return torch.where(active_mask, score, torch.zeros_like(score))


def ladder_entry_forward_velocity_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Keep the entry curriculum moving toward the physical lower timber.

    V3 removes the generic velocity-tracking reward after the approach so a
    policy cannot earn ladder progress by simply driving its trunk forward.
    The first two planned targets are different: without a small approach
    signal, the warm-start policy can stand on the floor forever and never
    discover the raised timber.  This term is therefore active only while an
    entry target is unresolved; it cannot advance the foothold state machine.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    # A capped positive velocity reward is intentionally weaker than the
    # contact milestone below.  It only supplies exploration direction and
    # does not make a high-speed collision a success.
    forward = torch.clamp(vx, min=0.0, max=cap)
    active_mask = (target < LADDER_ENTRY_FOOTSTEPS) & (target < active)
    return torch.where(active_mask, forward, torch.zeros_like(forward))


def ladder_entry_position_progress_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    """Provide a one-way approach potential before the first raised timber.

    The public MicroDuck step-up recipe keeps a forward command while the
    robot approaches the edge.  V3 intentionally disables generic velocity
    tracking after the approach, but a velocity-only entry term still admits
    a local optimum at the reset pose: a short initial burst is rewarded and
    then stopping is equally good.  This bounded potential supplies a dense
    position signal only until the two entry placements are complete.  It
    cannot advance the contact-gated foothold state machine and is therefore
    not a ladder-progress shortcut.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    asset = env.scene[asset_cfg.name]
    root_x = asset.data.root_link_pos_w[:, 0] - env.scene.terrain.env_origins[:, 0]
    # The reset root is about 40-60 mm before the lower timber.  The potential
    # saturates only after the trunk reaches the timber's center, so the
    # policy must actually enter the step-up workspace before this term fades.
    low = start_x - 0.06
    high = start_x + 0.02
    potential = torch.clamp((root_x - low) / max(high - low, 1e-4), 0.0, 1.0)
    previous = getattr(env, "_ladder_entry_position_potential_prev", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    fresh = env.episode_length_buf <= 1
    previous = torch.where(fresh, potential, previous)
    delta = potential - previous
    env._ladder_entry_position_potential_prev = potential.clone()
    active_mask = (target < LADDER_ENTRY_FOOTSTEPS) & (target < active)
    # Keep a small absolute residual as well as the potential difference.  A
    # pure delta is easy for PPO to forget after the first burst of motion;
    # the residual makes reaching the entry workspace preferable to standing
    # at reset, while the entry-only gate prevents root progress from becoming
    # a substitute for real rung contact later in the task.
    shaped = delta + 0.25 * potential
    return torch.where(active_mask, shaped, torch.zeros_like(shaped))


def ladder_foothold_contact_milestone(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    foot_lateral: float = 0.065,
    rung_surface_offset: float = _LADDER_FOOT_SITE_Z_OFFSET,
    target_tolerance: float = 0.060,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Reward the first valid contact for each planned foothold once.

    This is the sparse discovery signal used by the public MicroDuck step-up
    baseline, adapted to the V3 planner.  It only fires for an actual raw
    MuJoCo foot-to-entry/rung contact associated with the current target and
    never advances the target itself.  The state machine remains the sole
    source of task progress.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    along_height, lateral, _ = _ladder_foothold_errors(env, selected, goal)
    eligible = (
        (active > 0)
        & (target < active)
        & contact
    ...
    previous_target = getattr(env, "_ladder_contact_milestone_target", None)
    if previous_target is None or previous_target.shape != target.shape:
        previous_target = torch.full_like(target, -1)
    reset = env.episode_length_buf <= 1
    previous_target[reset] = -1
    reward = eligible & (previous_target != target)
    previous_target = torch.where(eligible, target, previous_target)
    env._ladder_contact_milestone_target = previous_target
    return reward.float()


def ladder_step_up_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    finish_x: float,
    finish_z: float,
    floor_foot_top_z: float = 0.025,
    sensor_name: str = "ladder_foot_contact",
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Teach the first swing to clear the physical lower timber.

    The video-aligned ladder has a real floor timber in front of rung 0.  A
    walking policy that only receives a root-alignment signal settles against
    that timber instead of discovering the required step-up motion.  This
    term is a smooth approach signal for the currently planned swing foot: it
    rewards clearing the timber and moving toward the rung.  It never
    advances the foothold state machine; actual rung contact plus the hold
    interval in ``_ladder_state`` remains mandatory.
    """
    target, active, _, _ = _ladder_state(
        env,
        rung_count,
    ...
    # The first rung is slightly behind the timber's rear edge.  Requiring a
    # foot to clear the timber top and reach the rung's x neighbourhood keeps
    # this shaping signal local to the actual first transition.
    lift = torch.sigmoid((selected[:, 2] - (floor_foot_top_z + 0.012)) / 0.012)
    forward = torch.sigmoid((selected[:, 0] - (target_pos[:, 0] - 0.035)) / 0.018)
    # This is a discovery signal, not a pose reward.  Paying the absolute
    # value every step creates a bad local optimum in which the policy keeps a
    # foot in the air or repeatedly hops without ever landing.  Use the
    # potential difference instead, with only a small residual for exploration;
    # the real rung contact and hold gate remain the only source of progress.
    potential = 0.60 * lift + 0.40 * forward
    previous = getattr(env, "_ladder_step_up_potential_prev", None)
    if previous is None or previous.shape != potential.shape:
        previous = potential.clone()
    fresh = env.episode_length_buf <= 1
    baseline = torch.where(fresh, potential, previous)
    delta = potential - baseline
    env._ladder_step_up_potential_prev = potential.clone()
    shaped = delta + 0.02 * potential
    return torch.where(
        (target < active) & (active > 0), shaped, torch.zeros_like(shaped)
    )


def ladder_phase_contact_velocity_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float,
    ...
    swing_time: float = 1.10,
    force_ref: float = 4.0,
    velocity_ref: float = 0.22,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    """Enforce the planned-footstep force/velocity gait from the reference.

    The AIST planned-footstep controller does not leave phase discovery to
    PPO.  During a single-support interval the planned swing foot must be
    unloaded and moving while the other foot carries load; the following
    double-support interval asks both feet to settle before the next target is
    exposed.  The force and velocity clocks use the same signed preference as
    the reference implementation: +1 means "high", -1 means "low".  This is
    deliberately a soft reward, while ``_ladder_state`` is still the hard
    transition gate and only advances on a real target contact.

    ``ladder_foot_contact`` sees only ladder geometry.  The union with the
    existing floor sensor keeps the same phase rule valid during the approach
    and when the support foot is still on the floor.
    """
    if cycle_time <= 0.0 or not 0.0 < swing_time < cycle_time:
        raise ValueError("expected 0 < swing_time < cycle_time")
    target, active, phase_time, _ = _ladder_state(
    ...
    ladder_sensor = env.scene.sensors.get(sensor_name)
    ground_sensor = env.scene.sensors.get(ground_sensor_name)
    if ladder_sensor is None and ground_sensor is None:
        return torch.zeros(env.num_envs, device=env.device)

    def _flags(sensor) -> torch.Tensor:
        if sensor is None:
            return torch.zeros((env.num_envs, 2), device=env.device)
        found = sensor.data.found
        if found.dim() == 3:
    ...
    # Use the raw foot-to-rung table for the contact phase.  The aggregate
    # sensor remains useful for force/velocity magnitudes, but its secondary
    # regex cannot identify a particular rung in a vectorized scene.
    contact = torch.maximum(
        _ladder_rung_foot_contact_flags(env, rung_count, sensor_name),
        _flags(ground_sensor),
    )

    def _force(sensor) -> torch.Tensor:
        if sensor is None or sensor.data.force is None:
            return torch.zeros((env.num_envs, 2), device=env.device)
        force = torch.nan_to_num(
            sensor.data.force, nan=0.0, posinf=0.0, neginf=0.0
        )
        # Contact sensors may expose one slot per primary geom or an extra
        # slot dimension.  Collapse only slot dimensions, preserving the
        # left/right primary-geom dimension used by the planner.
        while force.dim() > 3:
            force = force.amax(dim=-2)
        if force.dim() == 3 and force.shape[-1] == 3:
        ...
        if force.dim() != 2:
            return torch.zeros((env.num_envs, 2), device=env.device)
        if force.shape[1] == 1:
            force = force.expand(-1, 2)
        return force[:, :2]

    force = torch.maximum(_force(ladder_sensor), _force(ground_sensor))
    asset = env.scene[asset_cfg.name]
    speed = torch.linalg.vector_norm(
        asset.data.site_lin_vel_w[:, _ladder_ordered_site_ids(env, asset_cfg)], dim=-1
    )
    swing_id = _ladder_target_foot_id(target, rung_count)
    support_id = (swing_id + 1) % 2
    swing_contact = contact.gather(1, swing_id[:, None]).squeeze(1)
    support_contact = contact.gather(1, support_id[:, None]).squeeze(1)
    swing_speed = speed.gather(1, swing_id[:, None]).squeeze(1)
    support_speed = speed.gather(1, support_id[:, None]).squeeze(1)

    phase = torch.remainder(phase_time, cycle_time)
    in_single_support = (phase >= max(pre_swing_time, 0.0)) & (
        phase < max(pre_swing_time, 0.0) + swing_time
    )
    support_force = force.gather(1, support_id[:, None]).squeeze(1)
    swing_force = force.gather(1, swing_id[:, None]).squeeze(1)

    def _clock_score(
        value: torch.Tensor, desired: float, reference: float
    ) -> torch.Tensor:
        normalized = torch.clamp(value / max(reference, 1e-4), 0.0, 1.0)
        normalized = normalized * 2.0 - 1.0
        return torch.tan((math.pi / 4.0) * float(desired) * normalized)

    # Port the reference's signed force/velocity clock.  The contact flags are
    # retained as a safety gate so a stale force slot cannot masquerade as a
    # supporting foot after contact has ended.
    support_force_score = _clock_score(support_force, +1.0, force_ref) * support_contact
    swing_force_score = _clock_score(swing_force, -1.0, force_ref) * (1.0 - swing_contact)
    support_velocity_score = _clock_score(support_speed, -1.0, velocity_ref)
    swing_velocity_score = _clock_score(swing_speed, +1.0, velocity_ref)
    single_support_score = (
        support_force_score
        + swing_force_score
        + 0.65 * support_velocity_score
        + 0.65 * swing_velocity_score
    ) / 2.65
    double_support_score = (
        _clock_score(support_force, +1.0, force_ref)
        + _clock_score(swing_force, +1.0, force_ref)
        + 0.65 * _clock_score(support_speed, -1.0, velocity_ref)
        + 0.65 * _clock_score(swing_speed, -1.0, velocity_ref)
    ) / 2.65
    score = torch.where(in_single_support, single_support_score, double_support_score)
    return torch.where(
        (target < active) & (active > 0), score, torch.zeros_like(score)
    )


def ladder_foot_slip_cost(
    env: ManagerBasedRlEnv,
    sensor_name: str = "ladder_foot_contact",
    rung_count: int = 14,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
    ...
    raw_flags = _ladder_raw_rung_contact_flags(env, rung_count)
    if raw_flags is not None:
        found = raw_flags.any(dim=1).float()
    else:
        sensor = env.scene.sensors.get(sensor_name)
        if sensor is None:
            return torch.zeros(env.num_envs, device=env.device)
        ...
        if found.dim() == 3:
            found = found.any(dim=-1)
        found = (found[:, :2] > 0).float()
    asset = env.scene[asset_cfg.name]
    speed = torch.linalg.vector_norm(
        asset.data.site_lin_vel_w[:, _ladder_ordered_site_ids(env, asset_cfg), :2],
        dim=-1,
    )
    return (speed * found).sum(dim=-1)


def ladder_body_impact_cost(
    env: ManagerBasedRlEnv,
    sensor_name: str = "ladder_body_contact",
    threshold: float = 1.5,
) -> torch.Tensor:
    sensor = env.scene.sensors.get(sensor_name)
    if sensor is None or sensor.data.force is None:
        return torch.zeros(env.num_envs, device=env.device)
    force = torch.nan_to_num(sensor.data.force, nan=0.0, posinf=0.0, neginf=0.0)
    magnitude = torch.linalg.vector_norm(force, dim=-1).sum(dim=-1)
    return torch.clamp(magnitude - threshold, min=0.0)


def ladder_penetration_cost(
    env: ManagerBasedRlEnv,
    sensor_names: tuple[str, ...] = ("ladder_foot_contact", "ladder_body_contact"),
) -> torch.Tensor:
    total = torch.zeros(env.num_envs, device=env.device)
    for name in sensor_names:
        sensor = env.scene.sensors.get(name)
        if sensor is None or sensor.data.dist is None:
            continue
        # MuJoCo reports contact distance; a negative value means penetration.
        total += torch.clamp(-torch.nan_to_num(sensor.data.dist, nan=0.0), min=0.0).sum(dim=-1)
    return total


def ladder_body_collision_termination(
    env: ManagerBasedRlEnv,
    sensor_name: str = "ladder_body_contact",
    force_threshold: float = 2.0,
    top_clearance_x: float = 0.82,
    rung_count: int = 14,
    stage_schedule: tuple[tuple[int, int], ...] = (),
    asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
    ...
    force = torch.nan_to_num(sensor.data.force, nan=0.0, posinf=0.0, neginf=0.0)
    magnitude = torch.linalg.vector_norm(force, dim=-1).sum(dim=-1)
    hard_contact = _ladder_raw_trunk_hard_contact_flags(env)
    asset = env.scene[asset_cfg.name]
    origin = env.scene.terrain.env_origins
    root_x = asset.data.root_link_pos_w[:, 0] - origin[:, 0]
    # Stage A is the flat-ground standing/short-step bootstrap.  The ladder
    # is already present and collidable, but a hard body-hit termination here
    # would kill exploratory rollouts before the foothold curriculum begins.
    # The same discovery window is kept for the one-rung bootstrap: the first
    # physical problem is stepping over the lower timber, so a recoverable
    # brush must remain in the rollout long enough for PPO to discover a
    # swing.  Contact forces and penetration are still observed and penalized.
    active = _ladder_active_rungs(env, rung_count, stage_schedule)
    target = getattr(env, "_ladder_target_rung", None)
    if target is None or target.shape != (env.num_envs,):
        target = torch.zeros(env.num_envs, dtype=torch.long, device=env.device)
    exploratory_approach = (active <= 1) & (
        target < LADDER_ENTRY_FOOTSTEPS
    )
    effective_threshold = torch.where(
        exploratory_approach,
        torch.full_like(magnitude, force_threshold * 2.0),
        torch.full_like(magnitude, force_threshold),
    )
    # A walking-policy warm start can brush the first side rail before it has
    # adapted to the ladder.  Keep this opt-in and training-only: impact and
    # penetration rewards remain active, while final audits leave the strict
    # physical termination unchanged.
    if os.environ.get("MICRODUCK_LADDER_TOLERANT_BRIDGE") == "1":
        bridge_window = (active <= 2) & (target <= 1)
        effective_threshold = torch.where(
            bridge_window,
            torch.full_like(magnitude, force_threshold * 4.0),
            effective_threshold,
        )
    # Hard trunk contact is a failure while approaching or climbing.  The
    # top flip is intentionally exempt so landing can use the table surface.
    # The raw table is authoritative when available. The aggregate sensor
    # still supplies the force threshold, but crossbar contacts alone cannot
    # terminate an otherwise physical transfer.
    contact_gate = (
        hard_contact
        if hard_contact is not None
        else torch.ones_like(magnitude, dtype=torch.bool)
    )
    return (
        (active > 0)
        & (magnitude > effective_threshold)
        & contact_gate
        & (root_x < top_clearance_x)
    )


def ladder_foothold_deadline(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    start_x: float = 0.18,
    ...
    finish_z: float = 0.67,
    sensor_name: str = "ladder_foot_contact",
    first_target_deadline: float = 4.0,
    next_target_deadline: float = 2.0,
) -> torch.Tensor:
    """End an episode when a planned step is left unresolved for too long.

    The deadline is a curriculum guard against the stationary-support local
    optimum. It never creates foothold progress: the target must still pass
    the real contact, spatial alignment, and hold-time gate in ``_ladder_state``.
    The first target gets extra time to clear the entry timber; later rung
    exchanges use the shorter cadence expected by the planned-footstep policy.
    """
    target, active, phase_time, _ = _ladder_state(
        env,
        rung_count,
    ...
    deadline = torch.where(
        target < LADDER_ENTRY_FOOTSTEPS,
        torch.full_like(phase_time, float(first_target_deadline)),
        torch.full_like(phase_time, float(next_target_deadline)),
    )
    return (active > 0) & (target < active) & (phase_time >= deadline)


def ladder_top_stability_reward(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    table_x: float = 1.04,
    table_top_z: float = 0.70,
    asset_cfg: SceneEntityCfg = _LADDER_FEET_CFG,
) -> torch.Tensor:
    target, active, _, _ = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        0.18, 0.035, 0.72, 0.67,
    )
    asset = env.scene[asset_cfg.name]
    feet = asset.data.site_pos_w[:, _ladder_ordered_site_ids(env, asset_cfg)]
    in_table = (
        ...
        & (feet[..., 0] < table_x + 0.40)
        & (feet[..., 1].abs() < 0.40)
        & (feet[..., 2] > table_top_z - 0.045)
    )
    table_contact = _ladder_raw_table_foot_contact_flags(env)
    if table_contact is None:
        table_contact = torch.zeros(
            (env.num_envs, 2), dtype=torch.bool, device=env.device
        )
    on_table = (in_table & table_contact).any(dim=-1)
    speed = torch.linalg.vector_norm(asset.data.root_link_lin_vel_b[:, :2], dim=-1)
    stable = torch.exp(-speed.square() / 0.10**2)
    plan_count = _ladder_plan_count(rung_count)
    return on_table.float() * stable * (target >= plan_count).float() * (active >= plan_count).float()


def ladder_footstep_success(
    env: ManagerBasedRlEnv,
    rung_count: int,
    stage_schedule: tuple[tuple[int, int], ...],
    sensor_name: str = "ladder_foot_contact",
    ...
    """Finish a curriculum episode at its active foothold horizon.

    The full task additionally requires the table landing.  Earlier code
    always required all fourteen rungs, which made stage-B/C/D success
    impossible to observe and removed the episodic curriculum signal used by
    planned-footstep training.
    """
    target, active, _, _ = _ladder_state(
        env, rung_count, stage_schedule, sensor_name,
        0.18, 0.035, 0.72, 0.67,
    ...
    partial_stage_complete = (
        (active > 0)
        & (active < _ladder_plan_count(rung_count))
        & (target >= active)
    )
    full_task_complete = (
        (active >= _ladder_plan_count(rung_count))
        & (target >= _ladder_plan_count(rung_count))
        & (root[:, 0] > finish_x)
        & (root[:, 2] > table_top_z - 0.02)
        & (in_table & table_contact).any(dim=-1)
        & (cos_tilt > math.cos(math.radians(max_tilt_deg)))
    )
    return partial_stage_complete | full_task_complete
...
def ladder_success(
    ...
    """End an episode only after reaching the top while standing upright."""
    asset: Entity = env.scene[asset_cfg.name]
    origin = env.scene.terrain.env_origins
