...
# The first crossbar is the first real square-edged step above the lower
# timber.  Keeping it at the ladder origin makes that rise about 24 mm after
# the entry timber is lowered to 25 mm, matching the independently published
# MicroDuck step-up baseline while preserving the reference ladder silhouette.
LADDER_RUNG_START_FRACTION = 0.0
LADDER_RUNG_END_FRACTION = 0.95
TABLE_X = 1.04
TABLE_HALF_X = 0.48
TABLE_HALF_Y = 0.48
TABLE_TOP_Z = 0.70
...
def _add_video_ladder_copy(
    ...
    *,
    ...
    rung_count: int = 14,
    collision_mask: tuple[int, int] = (3, 3),
) -> None:
    ...
    geom_kwargs = {
        # Robot foot geoms use bit 1, while the walk model's body collision
        # geoms use bit 2.  Using both bits makes the ladder a real obstacle
        # for feet, legs, and trunk instead of a visual fence that the body
        # can tunnel through.
        "contype": collision_mask[0],
        "conaffinity": collision_mask[1],
        "friction": (1.15, 0.06, 0.01),
        "group": 0,
        # mujoco_warp's MULTICCD path rejects non-zero geom margins.  Keep
        # the geometric contact boundary exact and use solref/solimp below
        # for compliant impact handling instead.
        "margin": 0.0,
        "solref": (0.018, 1.0),
        "solimp": (0.88, 0.97, 0.001, 0.5, 2.0),
    }
    ...
    rung_fractions = tuple(
        LADDER_RUNG_START_FRACTION
        + (LADDER_RUNG_END_FRACTION - LADDER_RUNG_START_FRACTION)
        * rung_id
        / max(rung_count - 1, 1)
        for rung_id in range(rung_count)
    )
    for rung_id, t in enumerate(rung_fractions):
    ...
    _add_box(
        ...
        (LADDER_START_X + 0.07, 0.0, 0.0125),
        (0.08, 0.28, 0.0125),
        (0.58, 0.30, 0.08, 1.0),
        **geom_kwargs,
    )
