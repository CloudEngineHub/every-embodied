...
`8000` 端口通常依赖相应的示例应用已经启动，并非始终显示内容。如果设备可以 `ping` 通且 `7999` 页面可以打开，但 `8000` 暂无内容，可继续按第三章的按钮启动流程检查示例应用状态。
...
若设备仍使用厂商预置镜像的出厂设置，普通账户通常为 `sunrise`。登录时应按设备标签、随附文档或镜像发布说明输入初始口令；首次登录后应立即修改初始口令。

如果确实需要以 `root` 直接登录，并且当前镜像已经开放该登录方式，可执行：

```bash
ssh "root@${DEVICE_IP}"
```

部分厂商预置镜像也提供 `root` 出厂账户。只有在设备文档明确说明该账户已经启用时才使用，并按文档输入初始口令。日常开发仍建议使用普通账户配合 `sudo`，并在首次登录后修改所有仍启用的初始口令。
...
如果系统给 `sunrise` 配置了 `sudo` 权限，按终端提示输入当前账户口令即可完成提权。
