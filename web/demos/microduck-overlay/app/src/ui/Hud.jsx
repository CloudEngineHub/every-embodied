...
function Quickbar() {
  ...
  const locoWant = useGame((s) => s.locoWant);
  const visionEnabled = useGame((s) => s.vision.enabled);
  const cameraMode = useGame((s) => s.vision.cameraMode);
  return (
    <Box
      sx={{
        position: "fixed",
    ...
    >
      ...
      <HudPlate caption="Vision">
        <Box
          component="button"
          type="button"
          title="Toggle first-person ball chase"
          aria-label="Toggle autonomous first-person ball chase"
          aria-pressed={visionEnabled}
          onClick={() => { uiClick(); gameApi.setVisionAuto?.(!visionEnabled); }}
          sx={{
            ...hudHitSx,
            px: "1.05rem",
            background: visionEnabled ? ORANGE : "transparent",
            color: visionEnabled ? COMIC_INK : "rgba(250, 248, 242, 0.72)",
            "&:hover": {
              color: visionEnabled ? COMIC_INK : CREAM,
              background: visionEnabled ? ORANGE : "rgba(255, 122, 47, 0.12)",
            },
            "&:active": { filter: "brightness(0.92)" },
          }}
        >
      ...
      <HudPlate caption="Camera">
        <Box
          role="group"
          aria-label="Autonomous camera view"
          sx={{ display: "inline-flex", alignItems: "stretch", height: "100%", gap: `${CELL_GAP}px` }}
        >
          {[
            { name: "first", label: "1P", title: "First-person sensor view" },
            { name: "second", label: "2P", title: "Close follow view" },
            { name: "third", label: "3P", title: "Centered third-person chase view" },
          ].map(({ name, label, title }) => {
            const selected = cameraMode === name;
            return (
              <Box
                key={name}
                component="button"
                ...
                aria-label={title}
                aria-pressed={selected}
                onClick={() => { uiClick(); gameApi.setVisionCameraMode?.(name); }}
                sx={{
                  ...hudHitSx,
                  px: "0.72rem",
                  minWidth: 38,
