...
async function boot({ scene, camera, renderer }) {
  ...
  const sessions = {};
  const rdkBridge = requestedRdkBridge();
  let rdkProvider = null;
  ...
  // In RDK mode the browser never fetches or executes an ONNX policy. A
  // failed catalog handshake is fatal: there is deliberately no local
  // fallback that could make an RDK demonstration look successful.
  const donePolicies = bootLine("LOADING POLICIES");
  const sessionOpts = { executionProviders: ["wasm"] };
  try {
    if (rdkBridge) {
      rdkProvider = await RdkPolicyProvider.connect(rdkBridge, RDK_REQUIRED_POLICIES);
      setStore({ policyBrain: { name: "RDK X5", status: "online", latencyMs: 0 } });
  ...
  const visionSource = new VisionChaseSource({
    getVelocityLimits: () => velLims(),
    getMeasurement: () => getVisionMeasurement(),
    getBusyState: () => recovery ? "recovery" : mode,
    getManualOverride: () => padSource.isActive() || touchSource.isActive() || kbSource.isActive(),
  });
  // Physical inputs win, vision wins over a stale click-to-walk target,
  // and waypoint remains the zero-command fallback.
  const controller = new Controller({
    sources: [padSource, touchSource, kbSource, visionSource, waypointSource],
  });
  ...
  const activePolicySlot = () => {
    if (recovery?.state === "recovering") return "stand";
    return loco === "rollers" && mode === "walk" ? "drive" : mode;
  };
  const activeSession = () => sessions[activePolicySlot()];
  ...
  async function controlStep() {
    ...
    if (recovery?.state !== "fallen") {
      const observation = buildObs();
      let act;
      if (rdkProvider) {
        const remote = await rdkProvider.run(activePolicySlot(), observation);
        act = remote.actions;
        const now = performance.now();
        if (now - brainLastPush >= 250) {
          brainLastPush = now;
          setStore({
            policyBrain: { name: "RDK X5", status: "online", latencyMs: remote.latencyMs },
          });
        }
      } else {
        const feeds = { obs: new ort.Tensor("float32", observation, [1, OBS_SIZE]) };
        const out = await activeSession().run(feeds);
        act = out.actions.data;
      }
      lastAction.set(act);
  ...
  (async function controlLoop() {
    ...
    while (running) {
      try {
        ...
        running = false;
        if (rdkProvider) {
          setStore({ policyBrain: { name: "RDK X5", status: "offline", latencyMs: 0 } });
        }
        console.error("[policy] control stopped; no fallback", error);
        return;
      }
      count++;
      const now = performance.now();
  ...
  function ensureRollers() {
    rollersLoading ??= (async () => {
      ...
      const [rRig, , sDrive, sCrouch] = await Promise.all(jobs);
      if (!rdkProvider) {
  ...
  // Autonomous camera rig: first-person sensor view, close follow view, and a
  // stable centred third-person chase view. AUTO controls the robot; camera
  // choice is independent and defaults to the presentation-friendly 3P view.
  let autoCameraActive = false;
  let visionCameraMode = "third";
  const _visionSavedPos = new THREE.Vector3();
  const _visionSavedTarget = new THREE.Vector3();
  let visionSavedFov = camera.fov;
  let visionSavedNear = camera.near;
  ...
  const _autoDesired = new THREE.Vector3();
  const _autoForward = new THREE.Vector3();
  const _autoRight = new THREE.Vector3();
  let visionLastPush = 0;

  function setAutoCameraActive(value) {
    const next = !!value;
    if (next === autoCameraActive) return;
    if (next) {
      _visionSavedPos.copy(camera.position);
      _visionSavedTarget.copy(controls.target);
      visionSavedFov = camera.fov;
      visionSavedNear = camera.near;
      controls.enabled = false;
      chaseCam = false;
    } else {
      camera.position.copy(_visionSavedPos);
      controls.target.copy(_visionSavedTarget);
      controls.enabled = true;
      chaseCam = true;
      camera.fov = visionSavedFov;
      camera.near = visionSavedNear;
    }
    autoCameraActive = next;
    setVisionCameraMode(visionCameraMode, { push: false });
    camera.updateProjectionMatrix();
  }

  function setVisionCameraMode(mode, { push = true } = {}) {
    if (!new Set(["first", "second", "third"]).has(mode)) return;
    visionCameraMode = mode;
    if (autoCameraActive) {
      camera.fov = mode === "first" ? 62 : mode === "second" ? 54 : 48;
      camera.near = mode === "first" ? 0.015 : 0.03;
      camera.updateProjectionMatrix();
    }
    if (push) pushVisionState(true);
  }

  // Backward-compatible debug surface used by the recorder: true selects
  // first-person and false selects the default third-person view.
  function setVisionCamera(value) {
    setVisionCameraMode(value ? "first" : "third");
  }

  function updateVisionCamera() {
    if (!autoCameraActive) return;
    const qpos = data.qpos;
    const rawYaw = duckYaw(qpos);
    if (mode !== "roll" && !isKick()) chaseHeldYaw = rawYaw;
    const yaw = (mode === "roll" || isKick()) ? chaseHeldYaw : rawYaw;
    const fx = Math.cos(yaw);
    const fy = Math.sin(yaw);
    if (visionCameraMode !== "first") {
      _autoForward.set(fx, 0, -fy);
      _autoRight.set(fy, 0, fx);
      _autoTarget.set(qpos[0], qpos[2] + 0.13, -qpos[1]);
      if (visionCameraMode === "second") {
        _autoDesired.copy(_autoTarget)
          .addScaledVector(_autoForward, -0.68)
          .addScaledVector(_autoRight, 0.24);
        _autoDesired.y += 0.28;
      } else {
        // A small shoulder offset keeps the target ball visible instead of
        // placing it directly behind the duck, while lookAt(_autoTarget)
        // still pins the duck to the visual centre of the frame.
        _autoDesired.copy(_autoTarget)
          .addScaledVector(_autoForward, -1.22)
          .addScaledVector(_autoRight, 0.14);
        _autoDesired.y += 0.56;
      }
      const ease = visionCameraMode === "second" ? 0.12 : 0.09;
      camera.position.lerp(_autoDesired, ease);
      controls.target.lerp(_autoTarget, 0.18);
      camera.lookAt(controls.target);
      camera.updateMatrixWorld(true);
      return;
    }
    // Camera sits just ahead of the head shell. Trunk qpos is near the hip,
    // so a small z offset places the lens inside the orange bodywork.
    _visionEye.set(qpos[0] + fx * 0.12, qpos[2] + 0.30, -qpos[1] + fy * 0.12);
    // Aim at the ground roughly half a metre ahead. The previous one-metre
    // look target placed a close ball below the viewport just before contact.
    _visionLook.set(qpos[0] + fx * 0.68, 0.04, -qpos[1] + fy * 0.68);
    camera.position.copy(_visionEye);
    controls.target.copy(_visionLook);
    camera.lookAt(_visionLook);
    camera.updateMatrixWorld(true);
  }

  function projectedBallBox(measurement) {
    // The chase camera can put the ball directly behind the duck. A plain
    // projection would then draw a confident-looking box on the robot body,
    // even though the ball is occluded. Keep boxes on the sensor/debug views;
    // the 3P presentation view reports tracking state without a false box.
    if (
      !autoCameraActive
      || visionCameraMode === "third"
      || !measurement?.active
      || !measurement.visible
    ) return null;
    const qpos = data.qpos;
    _visionBall.set(qpos[ballQposAdr], qpos[ballQposAdr + 2], -qpos[ballQposAdr + 1]);
    _visionCam.copy(_visionBall).applyMatrix4(camera.matrixWorldInverse);
    const depth = -_visionCam.z;
    if (depth <= 0.02) return null;
    _visionNdc.copy(_visionBall).project(camera);
    if (Math.abs(_visionNdc.x) > 1.15 || Math.abs(_visionNdc.y) > 1.15) return null;
    const radiusY = BALL_RADIUS / (depth * Math.tan(THREE.MathUtils.degToRad(camera.fov) / 2));
    const radiusX = radiusY / camera.aspect;
    const clamp01 = (v) => Math.max(0, Math.min(1, v));
    return {
      left: clamp01((_visionNdc.x - radiusX + 1) / 2),
      right: clamp01((_visionNdc.x + radiusX + 1) / 2),
      top: clamp01((1 - (_visionNdc.y + radiusY)) / 2),
      bottom: clamp01((1 - (_visionNdc.y - radiusY)) / 2),
    };
  }
  ...
  function pushVisionState(force = false) {
    ...
    setStore({
      vision: {
        ...
        cameraMode: visionCameraMode,
        state: status.state,
        confidence: bbox ? Math.max(0.62, Math.min(0.99, 1.03 - (m.distance ?? 0) * 0.35)) : 0,
        bbox,
  ...
  function setVisionAuto(value) {
    const next = !!value;
    waypointSource.cancel();
    visionSource.setEnabled(next);
    setAutoCameraActive(next);
    if (next && !ballActive) spawnBall();
    pushVisionState(true);
  }
  ...
  function syncRig() {
    ...
    if (camResetT0 === null && !autoCameraActive) {
  ...
  window.rl = {
    ...
    controller, kbSource, padSource, visionSource,
    ...
    // Deterministic fall fixture for recovery QA and the recorded demo.
    debugFall: () => {
      const qpos = data.qpos;
      const qvel = data.qvel;
      const a = Math.PI * 0.47 / 2;
      qpos[2] = Math.max(qpos[2], 0.13);
      qpos[3] = Math.cos(a);
      qpos[4] = Math.sin(a);
      qpos[5] = 0;
      qpos[6] = 0;
    ...
    // Instant keyframe reset for deterministic policy/contact sweeps. This
    // intentionally bypasses the presentation respawn animation.
    debugResetInstant: () => {
      clearModeTimers();
      rollRun = kickRun = crouchRun = pickRun = null;
      postKickLock = 0;
      fallenSince = null;
      recovery = null;
      fallDebounce = 0;
  ...
  // Multiplayer is opt-in. Opening several local demo tabs used to make each
  // tab appear as an extra duck, which is confusing for single-agent QA.
