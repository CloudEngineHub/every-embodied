...
先在 PowerShell 中定义工作目录。后续命令都使用这两个变量，因此不依赖特定用户名或磁盘布局。

```powershell
if (-not $env:ROBOTICS_ROOT) { $env:ROBOTICS_ROOT = Join-Path $HOME "robotics" }
$env:SIM1_ROOT = Join-Path $env:ROBOTICS_ROOT "SIM1"
$env:GIT_BASH = Join-Path $env:ProgramFiles "Git\bin\bash.exe"
```

`$env:ROBOTICS_ROOT` 用于存放源码和运行结果；如果资产数据较大，应把它指向空间充足的磁盘。
...
```powershell
New-Item -ItemType Directory -Force $env:ROBOTICS_ROOT | Out-Null
Set-Location $env:ROBOTICS_ROOT
git clone --recurse-submodules https://github.com/InternRobotics/SIM1.git
Set-Location $env:SIM1_ROOT

micromamba create -n sim1 python=3.11 -y
micromamba run -n sim1 "$env:GIT_BASH" setup.sh
```
...
micromamba run -n sim1 "$env:GIT_BASH" download_assets.sh
```
...
micromamba run -n sim1 "$env:GIT_BASH" run_pipeline.sh --num 2 --workers 1
```
...
```powershell
micromamba run -n sim1 "$env:GIT_BASH" run_pipeline.sh --num 20 --workers 1
```

如果只是想观察生成轨迹，不想被过滤阶段移走文件，可以临时跳过过滤：

```powershell
micromamba run -n sim1 "$env:GIT_BASH" run_pipeline.sh --num 2 --workers 1 --skip_filter
```
...
```powershell
$env:SIM1_ASCII_ROOT = Join-Path $HOME "sim1_ascii"
```
...
```powershell
Set-Location "$env:SIM1_ASCII_ROOT"
$env:PYTHONNOUSERSITE='1'
...
```powershell
micromamba run -n sim1 "$env:GIT_BASH" components/render/batch_step4.sh replay/render_smoke_0001 "Fold the blue short shirt" 0 1
```
...
micromamba run -n py310-lerobot "$env:GIT_BASH" components/lmdb2lerobot/run_local.sh --src ./replay/render_smoke_0001/out_updated --out ./replay/render_smoke_0001/lerobot_dataset --keep-static-frames
```

如果环境缺少 `lmdb`、`scipy` 等依赖，先安装仓库提供的依赖文件：

```powershell
micromamba run -n py310-lerobot python -m pip install -r "$env:SIM1_ASCII_ROOT\components\lmdb2lerobot\requirements.txt"
```
