...
def _load_ckpt_and_infer_arch(args_override):
    ...
    if query is not None:
        inferred["chunk_size"] = int(query.shape[0])
        inferred["hidden_dim"] = int(query.shape[1])

    ff = state_dict.get("model.transformer.encoder.layers.0.linear1.weight")
    if ff is not None:
        inferred["dim_feedforward"] = int(ff.shape[0])

    action_head = state_dict.get("model.action_head.weight")
    if action_head is not None:
        inferred["action_dim"] = int(action_head.shape[0])

    robot_state = state_dict.get("model.input_proj_robot_state.weight")
    if robot_state is not None:
        inferred["state_dim"] = int(robot_state.shape[1])

    for key, value in inferred.items():
        old = args_override.get(key)
        if old != value:
            print(f"[ACT] overriding {key} from checkpoint: {old} -> {value}")
            args_override[key] = value
...
class ACTPolicy(nn.Module):
    ...
    def __init__(self, args_override, RoboTwin_Config=None):
        ...
        self.gripper_loss_weight = float(
            args_override.get(
                "gripper_loss_weight",
                os.environ.get("TRON2_ACT_GRIPPER_LOSS_WEIGHT", "1.0"),
            )
        )
        self.gripper_closed_loss_boost = float(
            args_override.get(
                "gripper_closed_loss_boost",
                os.environ.get("TRON2_ACT_GRIPPER_CLOSED_LOSS_BOOST", "1.0"),
            )
        )
        self.gripper_closed_threshold = float(
            args_override.get(
                "gripper_closed_threshold",
                os.environ.get("TRON2_ACT_GRIPPER_CLOSED_THRESHOLD", "0.5"),
            )
        )
        if self.gripper_loss_weight <= 0 or self.gripper_closed_loss_boost <= 0:
            raise ValueError("gripper loss weights must be positive")
        if self.gripper_loss_weight != 1.0:
            print(f"Gripper loss weight {self.gripper_loss_weight}")
        if self.gripper_closed_loss_boost != 1.0:
            print(
                "Gripper closed loss boost "
                f"{self.gripper_closed_loss_boost} threshold={self.gripper_closed_threshold}"
            )
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None):
        ...
        if actions is not None:  # training time
            ...
            if self.gripper_loss_weight != 1.0 or self.gripper_closed_loss_boost != 1.0:
                element_weights = torch.ones_like(all_l1)
                for idx in (7, 15):
                    if idx >= actions.shape[-1]:
                        continue
                    element_weights[..., idx] *= self.gripper_loss_weight
                    if self.gripper_closed_loss_boost != 1.0:
                        closed = actions[..., idx] < self.gripper_closed_threshold
                        boost = torch.where(
                            closed,
                            torch.full_like(actions[..., idx], self.gripper_closed_loss_boost),
                            torch.ones_like(actions[..., idx]),
                        )
                        element_weights[..., idx] *= boost
                all_l1 = all_l1 * element_weights
...
class ACT:
    ...
    def __init__(self, args_override=None, RoboTwin_Config=None):
        ...
        ckpt_path, ckpt_state_dict = _load_ckpt_and_infer_arch(args_override)
        if RoboTwin_Config is not None:
            for key in ("chunk_size", "hidden_dim", "dim_feedforward", "action_dim", "state_dim"):
                if key in args_override:
                    setattr(RoboTwin_Config, key, args_override[key])
        ...
        self.camera_names = args_override.get("camera_names", ["cam_high", "cam_right_wrist", "cam_left_wrist"])
        ...
        if ckpt_dir:
            ...
            ckpt_path = ckpt_path or os.path.join(ckpt_dir, ckpt_name)
            print("current pwd:", os.getcwd())
            if ckpt_state_dict is not None or os.path.exists(ckpt_path):
    ...
    def get_action(self, obs=None):
        ...
        # Stack images in the same semantic order used during training.
        camera_key_map = {
            "cam_high": "head_cam",
        ...
        for cam_name in self.camera_names:
            curr_images.append(obs[camera_key_map.get(cam_name, cam_name)])
