...
本章面向已经掌握导航基础概念、希望完成一次端到端实践的读者，重点梳理感知、地图、规划和控制模块之间的数据流。

课程使用轻量测试环境，将算法拆分为独立文件并由主函数组合调用。理解各模块的输入和输出后，可以替换感知、规划或控制实现，也可以将模块接入其他仿真环境与真实导航系统。

后续内容依次介绍环境安装、模型准备、代码运行和模块实现，形成从视觉感知、决策到路径规划与运动控制的完整基线。
...
```python
export NAV_ROOT=/path/to/navigation-workspace
mkdir -p "$NAV_ROOT/datasets" "$NAV_ROOT/models"
python -m habitat_sim.utils.datasets_download --uids habitat_test_scenes --data-path "$NAV_ROOT/datasets"
```


如果使用远程模型接口，环境配置到此完成。需要在本地运行视觉语言模型时，继续安装下面的依赖并下载权重。
...
将模型放在独立子目录中，可避免权重文件与项目源码混在一起。显存较小时先使用 4B 版本；替换为 8B 或其他模型时，同步更新 `vlm/`、`navigator.py` 和 `config.py` 中的模型路径与推理参数。
