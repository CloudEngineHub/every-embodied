...
它应该放在 `06-策略抓取或抓取VLA/大模型控制、VLA、VLM`。原因是 LWD 的训练对象是 VLA policy，核心问题是机器人操作策略如何从真机部署数据中持续变强；它不是世界模型，也不是纯仿真 benchmark。
...
| 代码 | 官方项目页当前未展示独立代码仓库入口 | 更适合做方法阅读、系统拆解和复现设计，不建议写成一键复现教程 |
...
当前不建议写成“下载代码、运行训练”的教程。更合理的复刻路线是分三档。
