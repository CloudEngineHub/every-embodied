...
本节的可复现入口由以下脚本组成。脚本使用变量接收数据、模型和输出路径，凭据从环境变量或交互输入读取。
...
1. **第 06 章技术正文**：讲解诊断方法、复现脚本和实验口径。
2. **专题组队学习入口**：把环境、数据、ACT 诊断、SmolVLA 对照和 pi_0 短链路检查拆成按周完成的任务。

读者可以先按“采集→训练→部署”完成单模型主线，再用组队学习任务做多模型对照。
