...
G0.5 的主要贡献位于 VLA 结构和动作表征：同一个自回归模型联合处理视觉记忆、结构化推理和动作序列，并用统一动作空间连接不同机器人本体。它不承担未来视频预测或独立导航规划，而是把操作策略内部原本分散的输入、推理和动作接口合并为一条生成链路。
...
G0.5 的完整预训练需要大规模数据和计算资源。本节将复现目标分为三档，使读者可以先验证公开权重和接口，再逐步进入微调与完整训练。
