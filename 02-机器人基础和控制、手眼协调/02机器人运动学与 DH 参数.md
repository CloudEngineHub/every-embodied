...
本章使用标准 DH 参数建立串联机械臂的连杆坐标系，并通过矩阵连乘计算末端位姿。代码示例采用二连杆平面机械臂，运行后应得到末端位置 `[1, 1, 0]`。
...
- 建表前先确认使用标准 DH 还是改进 DH，两者的坐标系约定和矩阵顺序不同。
- 旋转关节以 $\theta_i$ 为变量，移动关节以 $d_i$ 为变量。
- 每个相邻变换矩阵都应单独核对，最后再按连杆顺序连乘。
