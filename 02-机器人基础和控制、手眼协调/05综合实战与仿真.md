...
本章包含两个相互独立的练习：根据标准 DH 参数计算 UR5 末端位姿，以及对阻尼单摆进行数值积分并观察时域响应和相图。前者检查运动学建模，后者检查动力学方程是否能转化为可运行仿真。
...
- UR5 参数表使用标准 DH 约定，不能直接与改进 DH 参数表混用。
- 阻尼为正时，单摆机械能逐步衰减，相图应向稳定平衡点收缩。
- 修改初始角度、阻尼和仿真时长，观察线性近似与大角度非线性运动的差异。
