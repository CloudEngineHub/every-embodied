...
本章推导二连杆平面机械臂的雅可比矩阵，并分别验证关节速度到末端速度、末端广义力到关节力矩的映射。代码还演示了如何用位置雅可比判断机械臂伸直时的奇异位形。
...
- 先明确任务使用位置、姿态还是完整位姿，再选择对应的雅可比行。
- 非方阵雅可比不能直接计算行列式；可使用任务相关子矩阵、秩或奇异值判断退化。
- 力映射采用 $\tau=J^TF$，其中 $F$ 的维度必须与末端速度表示一致。
