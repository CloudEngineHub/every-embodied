...
Source: 06-策略抓取或抓取VLA/大模型控制、VLA、VLM/05DiT4DiT-LIBERO/01DiT4DiT-LIBERO训练与评估.md
Source SHA-256: 1d79ccab5a86a29c2628a5f0d041e77361da26424e29b055e47e27b780822e11
...
A key principle of this chapter is that large models and conda environments can reuse the paid shared disk, while training data, logs, and teaching outputs are stored on the free data disk. This ensures the process runs smoothly without leaving the free disk full with checkpoints of 20GB due to smoke tests.

| Type | Measured Path | Purpose |
| :-- | :-- | :-- |
| Project Root Directory | `/root/gpufree-share/dit4dit_eval` | DiT4DiT repository, LIBERO repository, model cache |
| DiT4DiT Environment | `/root/gpufree-share/conda-envs/dit4dit` | policy server and training scripts |
| LIBERO Environment | `/root/gpufree-share/conda-envs/libero` | MuJoCo / robosuite simulation evaluation |
| Free Dataset Disk | `/root/gpufree-data/dit4dit_train_test` | single suite dataset, training logs, smoke test output |
| Cosmos Backbone | `/root/gpufree-share/dit4dit_eval/models/Cosmos-Predict2.5-2B` | gated Hugging Face model |
| DiT4DiT Checkpoint | `/root/gpufree-share/dit4dit_eval/models/dit4dit-model/dit4dit_libero/final_model/pytorch_model.pt` | pre-trained policy weights |

If you use your own server, you can keep this idea, but replace the path with your mount drive. It is recommended to prepare at least 48GB of VRAM for single-card evaluation and smoke training. The RTX 4090 may also be able to run some processes, but 24GB of VRAM is more likely to encounter OOM issues.

Here, we would like to emphasize the disk planning. The models of DiT4DiT, Cosmos backbone, and conda environments have large weights, so they are suitable for storage on a larger shared or long-term disk. The data, logs, and temporary outputs used for training smoke tests should be stored on a cheaper data disk. If everyone puts all the content directly on the system disk, `No space left on device` errors are likely to occur. Moreover, these errors often appear during pip installation, HF download, or checkpoint saving, making troubleshooting time-consuming.
...
It is recommended to save a private env file on the server:
...
If the output is `HF_TOKEN=set`, it means the environment variable has taken effect; if it is still `missing`, check whether there is no `source /root/.secrets/codex_tokens.env`, or whether the login shell does not automatically load this file.
