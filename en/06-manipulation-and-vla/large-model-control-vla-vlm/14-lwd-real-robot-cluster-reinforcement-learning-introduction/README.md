...
## 1. How LWD Turns Deployment into a Learning Loop
...
```text
Offline-pretrained VLA
    -> deploy to multiple real robots
    -> collect successes, failures, recoveries, interventions, and rollouts
    -> update one shared VLA with offline-to-online RL
    -> redeploy the improved policy
```
...
LWD trains a generalist VLA policy and asks how a robot manipulation policy can continue improving from real deployment data. It neither predicts future scenes nor limits itself to simulation benchmarks; its focus is the offline-to-online reinforcement-learning loop built from fleet experience.
...
| Code | No independent code repository entry displayed on the official project page currently | Supports method study, system analysis, and reproduction planning |
...
Because the complete training repository and real-robot deployment scripts are not public, the reproduction route is divided into three levels.
