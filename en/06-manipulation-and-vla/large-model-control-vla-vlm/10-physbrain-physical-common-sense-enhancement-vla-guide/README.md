...
It is suitable to be placed in the `06-策略抓取或抓取VLA/大模型控制、VLA、VLM` section of this repository, after OpenVLA, DiT4DiT, EventVLA, WALL-OSS/WALL-X, and 3DVLA. The reason is straightforward: PhysBrain 1.0 ultimately serves robot control and post-training VLA, so it is not a world model that can be used like a simulation tool `step(action)`, nor is it a neural world model specifically designed for video rollout.

After completing this section, everyone needs to grasp a key judgment:

> The key to PhysBrain 1.0 is not “imitating actions from more robot trajectories”, but rather compiling large-scale human first-person videos into structured physical常识 supervision, training a VLM that understands physical relationships, contact processes, spatial distances, and action feasibility, and then adapting it through ability preservation and language sensitivity to transfer this physical prior knowledge into robot action generation.
...
It cannot be simply understood as “PhysBrain trained a world model”. It did not generate an interactive environment, nor updated the policy during the imagined rollout. Instead, it acts like adding a pre-training stage of physical常识 before VLA, and then protects this常识 from being forgotten during VLA fine-tuning through architectural and loss design.
...
| Simulation Control | SimplerEnv-WidowX, SimplerEnv-GoogleRobot | Whether physical常识先验 can be transferred to desktop manipulation and OOD scenarios |
...
Second, many strong outcomes depend on robot adaptation specific to the benchmark. PhysBrain learns physical常识 initially, but the resulting executable policy still requires robot data and specific embodiment adaptation. It cannot be understood as “just watching human videos and directly controlling any robotic arm”.
...
| :--- | :--- | :--- |
| Robot data is too expensive | Continue to expand robot trajectory collection | First learn physical常识 using human first-person videos |
