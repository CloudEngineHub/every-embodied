...
| Item | Information |
| :--- | :--- |
| Item Homepage | [EVA-Client Project Page](https://colalab.net/projects/eva-client) |
...
| Python Version | `>=3.10,<3.12`, recommended Python 3.10 |

EVA-Client is not aimed at "retraining a model," but at the following high-frequency issues:

1. How can the camera status, joint states, and task instructions be sent to the policy in a consistent format?
2. The policy returns an entire action chunk at a time. How can the robot execute it continuously without frequent pauses?
...
OpenPI, StarVLA, GR00T, and DreamZero in the center of the figure represent compatible policies or training ecosystems, but do not indicate that EVA-Client includes these training algorithms. The policy clients that can be directly seen in the current code include:
...
After training, the checkpoint is launched by the corresponding framework into a policy service. EVA-Client is responsible for organizing observations, requesting actions, and executing the returned action blocks.

### 2.4 Deployment: Action block scheduling determines whether it "works" or "runs smoothly"

Synchronous execution in the diagram causes a pause during the wait for the next inference; asynchronous execution allows inference and robot control to overlap, enabling seamless transition between old and new action blocks. The lag, action jumps, and accumulated delay issues on the real robot usually occur at this layer.
...
The transport layer is responsible for receiving the camera and robot states from the execution backend, and sending control commands. The current project provides an offline loading interface for ROS1, ROS2, ZMQ, and LeRobot dataset datasets.

Separating transmission and policy has two direct benefits: the same policy can be switched to ROS/ZMQ backends; the same robot can also switch between real hardware, offline data, or mock environments for layered troubleshooting.

### 3.2 Robot Definition: Define ontology contract
...
The Policy Client is responsible for three tasks: converting unified observations into model inputs; sending requests to remote or local inference services; and restoring the response into a unified action chunk. The model's tokenizer, checkpoint loading, and GPU inference can continue to remain in the original training framework.
...
| `naive` | Asynchronous request; directly replaces the old buffer when a new block arrives | Fast throughput verification | Boundary actions may jump |
...
If the action chunk length is $H$, the control frequency is $f_c$, and the single inference delay is $t_i$, synchronous execution will easily stop when $t_i$ approaches or exceeds the remaining buffer. The essence of the asynchronous policy is to complete the next inference in advance while the robot performs the current action block, and to ensure the timing alignment between the two actions.

### 3.5 Recorder and Evaluator: Turning "success rate" into a evidence chain

The recorder preserves the observations, model predictions, actual execution commands, and evaluation metadata. The actual execution actions are not necessarily equal to the original predictions: they may be filtered, IK-processed, smoothed, intervened by humans, or subjected to safety checks. By recording these different quantities separately, it is possible to determine where the failure originates—whether from the model, communication, scheduling, or the controller.

## 4. What tasks correspond to the seven manipulation pages
...
It is recommended to enter the real robot closed loop in the order of `MANUAL -> COLLECT/REPLAY -> DEBUG -> EVAL`. Do not run the policy continuously without verifying the camera, joint sequence, and emergency stop first.

## 5. Data and Logs: Why It Can Be Connected to LeRobot
...
The repository also lists some robots as under development or pending addition. The support list will be updated as the project develops. Before deployment, it is required that the three entries `README.md`, `configs/`, and `examples/hardware/` from the official repository exist simultaneously, rather than relying solely on the project promotional images.

### Can SO-101 be used directly currently?

It cannot be written as "supported". In Issue #94, the author stated on 2026-07-09 that it would be supported later; however, as of the official submission `ff36587`, no description of the robot, hardware nodes, and operating configurations for SO-101 have been found in the repository.
...
2. Implement hardware execution nodes using the LeRobot/Feetech interface, and expose observations and actions via existing ZMQ or a suitable transport;
...
6. Conduct integration tests using a fake policy and recorded data, followed by integration with a real VLA service.
...
- Whether playback, pause, and episode switching are functional.
...
When facing a new configuration, do not just change the IP. It is recommended to check in the following order:

| Configuration Block | Required Content |
| :--- | :--- |
| Robot | Body type, joint group, camera, initial pose, URDF |
...
If "the interface shows images but the robot does not move", prioritize checking the action space, joint order, and execution nodes; if "the robot stops every once in a while", prioritize checking inference delay, chunk consumption speed, and scheduling strategy; if "the actions suddenly change", prioritize comparing the original model actions, the boundary between old and new chunks, and the actual actions sent.

## 10. Recommended reproduction route

This tutorial does not provide real robot button-by-button replication, but it is recommended to proceed from low to high risk:

1. Run the open-loop example of the dataset provided by the repository successfully;
...
1. First, study the sections related to [RDK-X5 connecting the LeRobot robotic arm for remote manipulation](../../../../03-机器人硬件、lerobot及地瓜RDK-X5开发板控制教程/03RDK-X5连接lerobot机械臂进行遥操作.md) or data collection within the warehouse, and understand episode and multi-modal time alignment;
2. Then, complete the training and reasoning of [SmolVLA](../../../../06-策略抓取或抓取VLA/大模型控制、VLA、VLM/01SmolVLA-LIBERO/01SmolVLA-libero.md), [OpenVLA](../../../../06-策略抓取或抓取VLA/大模型控制、VLA、VLM/02OpenVLA复现/02openvla复现.md) or [OpenPI / π0](../../../../06-策略抓取或抓取VLA/大模型控制、VLA、VLM/04mujoco复现ACT、Pi0、SmolVLA);
3. Use EVA-Client to connect to a real ontology, handling policy services, action block scheduling, logs, and evaluations;
