# Hf Fast Download Avoids Downloading Unnecessary Other Branch Files Simultaneously

Quickly download download\_hf\_files.py

```bash
export HF_DOWNLOAD_ROOT="${HF_DOWNLOAD_ROOT:-$HOME/huggingface-downloads}"
mkdir -p "$HF_DOWNLOAD_ROOT"
```

python3 download\_hf\_files.py IPEC-COMMUNITY/spatialvla-4b-224-pt main --repo-type model --download\_path "$HF_DOWNLOAD_ROOT/spatialvla-4b-224-pt"
...
python3 download\_hf\_files.py nikriz/aopoli-lv-libero_combined_no_noops_lerobot_v21 main --repo-type dataset --download\_path "$HF_DOWNLOAD_ROOT/aopoli-lv-libero"

python3 hf_downloader.py nikriz/aopoli-lv-libero_combined_no_noops_lerobot_v21 main --repo-type dataset --download\_path "$HF_DOWNLOAD_ROOT/aopoli-lv-libero-new"
...
You need to switch to the path where the session is located to resume breakpoint transfer.
