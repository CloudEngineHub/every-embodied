# Updating the Video2Robot Body-Model Download Script

The following replacement adds resumable downloads and supports either `aria2c` or `wget` when retrieving the required SMPL-X and SMPL model files.

Run the following commands from the Video2Robot repository root. If needed, set `VIDEO2ROBOT_ROOT` before entering the directory:

```bash
export VIDEO2ROBOT_ROOT="${VIDEO2ROBOT_ROOT:-$HOME/video2robot}"
cd "$VIDEO2ROBOT_ROOT"
```

The script to replace is `third_party/PromptHMR/scripts/fetch_smplx.sh`.
