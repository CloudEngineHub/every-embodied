...
Source: 17-具身世界模型/2、VAE详解与代码实现/VAE详解与代码实现.md
Source SHA-256: 015c11b2fc68e466cc949dc9590be3d88807baf2def7fecabbff248af395d599
Model: tencent/Hy-MT2-1.8B-GGUF:Q4_K_M
Review machine-translated technical claims before relying on them.
-->
# Detailed Explanation of VAE and Implementation of MNIST Image Generation

### This tutorial provides a comprehensive explanation from the motivation behind VAE, probability models, ELBO derivation, and final training objectives, to the code implementation for generating MNIST images. The focus is on understanding why VAE is designed in this way, and how to determine whether the model has truly learned a sampleable latent space during training and generation.
...
The generative model aims to learn the real data distribution:
...
### 1.2 Problems of Ordinary Autoencoder

The structure of a regular Autoencoder is:
...
Ordinary AE is suitable for compression, dimensionality reduction, noise removal, and feature learning, but it is not inherently a generative model. This is because the Encoder output of an ordinary AE is a fixed point:

$$
z=f_{\phi}(x)
$$

This only ensures that the training samples can be mapped near certain latent variable points, but it does not guarantee that the entire latent space is continuous, regular, and samplable.
...
3. When performing interpolation of潜 variables between two samples, the middle region may be meaningless.

The normal AE can be reconstructed, but its generation is not guaranteed.
...
- Numerical integration for each sample is extremely costly.
...
This chapter only discusses two derivation routes. They yield the same ELBO, but the questions they answer are different.

- **Route 1: Jensen's Inequality Approach**
  Answer the question, "Why can an unsolvable $\log p_{\theta}(x)$ be transformed into a feasible lower bound?"

- **Route 2: Likelihood Decomposition Route**
  Answering "What exactly is the difference between ELBO and the real $\log p_{\theta}(x)$?".

### 2.1 Route 1: Obtain a trainable lower bound from Jensen's inequality

The goal of this route is to start from the training objectives and construct an optimizable lower bound.
...
The problem here is that the integrals are unsolvable. Since there is only $x$ training data, and the latent variable $z$ is not observable, all possible $z$ integrals must be taken into account.

Step 1: Multiply or divide by a distribution $q_{\phi}(z|x)$ in the integral:
...
Next, expand the ELBO into a feasible loss. From the joint distribution:
...
- $D_{KL}(q_{\phi}(z|x)\|p(z))$: A Priori constraint, encouraging the Encoder to produce a distribution close to the standard normal distribution.
...
If the network output is $\log\sigma^2$, denoted as `logvar`, it is usually written as:
...
This approach does not use the Jensen inequality initially, but instead splits the true log-likelihood into two parts. The goal is to explain why ELBO represents a lower bound, and how far this lower bound is from the true objective.
...
- $D_{KL}(q_{\phi}(z|x)\|p_{\theta}(z|x))$: The theoretical posterior gap, used to explain the difference between ELBO and the true likelihood. It cannot be directly trained because the true posterior is intractable;
...
The dataset uses the mnist dataset and will be downloaded automatically. There is no need to worry about issues with the dataset; you just need to install torch properly.

The model here uses a U-Net-style convolutional downsampling and upsampling structure. Note that during the generation phase, there are only random latent variables $z$, and no input images are provided, so it cannot rely on cross-layer skip features of the input images. The code adopts a "U-Net-style hierarchical convolutional structure" instead of relying on the full U-Net with input skip features.
...
This randomness comes from `eps`, and `z` is a differentiable function of `mu` and `logvar`. The gradient can be transmitted back from the Decoder to the Encoder.

After re-parameterization, what is actually calculated during model training is a differentiable estimate of the negative ELBO. For a training sample $x$, the calculation process can be written as:
...
After ignoring constants unrelated to model parameters, the reconstruction term is equivalent to the mean squared error. The formula for true optimization can be written as:
...
MNIST pixels are normalized to $[0,1]$, so the reconstruction term uses binary cross-entropy. The logits version is used in the training script:
...
In this way, the model first learns basic reconstruction, and then gradually strengthens the潜 space constraints, which can alleviate the KL disappearance.

### 3.4 Output of the training script
...
The generation script does not require training data; it only relies on the trained Decoder.
...
2. **Reconstruction loss and KL loss should be recorded separately**
   It is not sufficient to only look at the total loss. When KL approaches 0, it may indicate that latent variables are not being used.
...
5. **Do not blindly increase the latent variable dimensions**
   For MNIST, you can start with 16, 32, or 64. Too small a dimension will result in poor reconstruction, while too large a dimension may leave some dimensions unused.
...
The skip connection in the traditional U-Net extracts features from the input image and passes them to the Decoder. During the VAE generation phase, there is no input image, only random $z$s, so this input feature cannot be relied upon. Here, a U-Net-style convolutional downsampling and upsampling structure is used.
...
The value of VAE lies in the fact that it not only enables the model to reconstruct inputs, but also organizes data into a continuous, regular, and sampleable latent space. This latent space makes random generation, interpolation, and representation learning possible.

---

## 6. After-class Exercises

1. Why cannot a regular AE ensure effective random sampling?
