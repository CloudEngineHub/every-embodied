...
## 1. From Video Generation to Closed-Loop World Simulation
...
```text
Multi-view video history + robot action trajectory
    -> action-conditioned video world model
    -> future multi-view rollout video
       + predicted proprioceptive state
       + optional task reward interface
```

The central question in GE-Sim 2.0 is not how a policy outputs actions, but whether a learned world model can generate action consequences like a simulator and support closed-loop execution, evaluation, and learning. This is why the method is analyzed as a world simulator rather than as an action policy.
