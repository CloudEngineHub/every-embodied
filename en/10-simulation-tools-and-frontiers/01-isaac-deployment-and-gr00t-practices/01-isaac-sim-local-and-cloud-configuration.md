...
Source: 10-具身智能其他仿真工具及仿真前沿/01Isaac部署与GR00T实践/01Isaac-Sim本地与云端配置.md
Source SHA-256: f5df1b70b4dca590cb39b51a3c978fffe7a34e5978cdbed0049ee1fde260392d
...
For beginners using Windows, the Workstation installation package is highly recommended, as it is more similar to the usage of ordinary desktop software. You can download it via a browser or directly in PowerShell. The installation package is about several dozen GB in size. It is recommended to check the disk space first, and then extract it to short paths like `C:\isaacsim` to avoid installation and cache issues caused by long Windows paths.
...
If the pip installation on Windows reports a too-long path, enable long-path support first, or use a shorter environment path. For workstation installations, it is also recommended to use short paths such as `C:\isaacsim`, rather than deeply nested personal document directories.
