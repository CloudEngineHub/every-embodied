...
Define the source, environment, and cache roots before installation. The default keeps everything under the current user's home directory; override `EBENCH_ROOT` when a larger data disk is available:

```bash
export EBENCH_ROOT="${EBENCH_ROOT:-$HOME/ebench-repro}"
