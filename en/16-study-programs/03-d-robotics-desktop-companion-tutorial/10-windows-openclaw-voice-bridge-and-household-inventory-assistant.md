...
Define the workspace once in PowerShell. Every later command uses these variables instead of a machine-specific user directory.
...
The local query entry point is `$PROJECT_ROOT\tools\household_inventory_cli.py`.

Typical commands are as follows:

```bash
python "$PROJECT_ROOT\tools\household_inventory_cli.py" top --limit 3 --format text
python "$PROJECT_ROOT\tools\household_inventory_cli.py" locate --name scissors --format text
python "$PROJECT_ROOT\tools\household_inventory_cli.py" create-order --name apples --send-feishu --format text
```
...
The board-side light controller is `$PROJECT_ROOT\bin\magicbox-hw`.
