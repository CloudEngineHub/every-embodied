...
| **第三阶段** | 模仿学习、大模型(VLA/VLN)、Sim2Real | 复现大模型导航VLN、OpenVLA、SmolVLA 等前沿项目，实现仿真或真实部署 |
  ...
  ## 🔥 News & Highlights
  - **[2026-07-18]** 新增 [3DVLA 三维空间实例增强 VLA 导读](./06-策略抓取或抓取VLA/大模型控制、VLA、VLM/09-3DVLA三维空间实例增强VLA导读/README.md) 与 [HumanoidMimicGen 全身规划数据生成导读](./10-具身智能其他仿真工具及仿真前沿/12HumanoidMimicGen全身规划数据生成导读/README.md)：前者归入现有 VLA 章节，重点拆解北大 3DVLA 如何把多视角 3D 空间、实例和遮挡补全 token 即插即用地注入 action expert；后者归入仿真与数据生成章节，系统讲解 NVIDIA 等提出的少量人形遥操作示教、技能约束、全身 IK/运动规划、G1 loco-manipulation benchmark 和 sim-and-real co-training 路线。
