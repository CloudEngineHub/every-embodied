...
def main(args):
    ...
    # Keep the official default, but let checkpoint fine-tunes reduce the
    # visual backbone together with the policy instead of silently updating it
    # at 1e-5 while the requested policy learning rate is much smaller.
    lr_backbone = float(os.environ.get("TRON2_ACT_LR_BACKBONE", "1e-5"))
    backbone = "resnet18"
    if policy_class == "ACT":
        enc_layers = 4
        dec_layers = 7
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    policy = make_policy(policy_class, policy_config)
    init_ckpt = os.environ.get("ACT_INIT_CKPT", "").strip()
    if init_ckpt:
        state_dict = torch.load(init_ckpt, map_location="cpu")
        loading_status = policy.load_state_dict(state_dict)
        print(f"Initialized policy weights from {init_ckpt}")
        print(f"Initial loading status: {loading_status}")
    policy.cuda()
    ...
    min_val_loss = np.inf
    best_ckpt_info = None
    rolling_best_save_freq = int(os.environ.get("ACT_BEST_SAVE_FREQ", "0"))
    rolling_last_save_freq = int(os.environ.get("ACT_LAST_SAVE_FREQ", "0"))
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if rolling_best_save_freq > 0 and is_main and best_ckpt_info is not None:
            if epoch == 0 or (epoch + 1) % rolling_best_save_freq == 0:
                _, _, best_state_dict = best_ckpt_info
                ckpt_path = os.path.join(ckpt_dir, "policy_best.ckpt")
                torch.save(best_state_dict, ckpt_path)
        if rolling_last_save_freq > 0 and is_main and (epoch + 1) % rolling_last_save_freq == 0:
